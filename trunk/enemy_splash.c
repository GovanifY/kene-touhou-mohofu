...
void enemy_splash_add(int lv)
{
	...
	s=sprite_add_file("splash.png",20,PR_ENEMY);
	s->type=SP_EN_SPLASH;
...
void enemy_splash_move(SPRITE *s)
{
	...
	case 1:		//wait
		...
		else
		{
			...
			else if((p>=-3*M_PI/8)&&(p<-M_PI/8))
				s->aktframe=s->aktframe%4+12;
			else if((p>=-7*M_PI/8)&&(p<-5*M_PI/8))
				s->aktframe=s->aktframe%4+4;
			else if((p>=7*M_PI/8)||(p<-7*M_PI/8))
				s->aktframe=s->aktframe%4;
			d->wait--;
		}
		break;
	case 2:		//shot
