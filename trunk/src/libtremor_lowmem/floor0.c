...
static const ogg_uint16_t barklook[54/*+2*/ /*.align*/] =
{
...
void vorbis_lsp_to_curve(
	ogg_int32_t *curve,int n,int ln,
	...
	if (ampraw>0)	/* also handles the -1 out of data case */
	{
		...
		if ((booknum!=-1) && (booknum<info->numbooks) ) /* be paranoid */
		{
			...
			ogg_int32_t last		= 0;
			{
				...
				{
					if (vorbis_book2_decodev_set(b, &vd->opb, b->dim, -24, lsp+j)==-1)
					{
						goto eop;
					}
				}
