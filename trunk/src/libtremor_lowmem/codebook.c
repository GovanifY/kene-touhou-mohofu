...
static ogg_uint32_t decpack(
	long entry,long used_entry,long quantvals,
...
void vorbis_book2_clear(codebook *b)
{
...
int vorbis_book2_unpack(codebook *s, oggpack_buffer *opb)
{
	...
	case 0:
		...
		/* this is the correct boundary here; we lose one bit to
			 node/leaf mark */
		s->dec_nodeb = _determine_node_bytes(s->used_entries, (_ilog(s->entries)>>3)+1);/*/8*/
		s->dec_leafw = _determine_leaf_words(s->dec_nodeb, (_ilog(s->entries)>>3)+1);/*/8*/
		s->dec_type=0;
	...
	case 1:
		...
		{
			/* packed values */
			long total1=((s->q_bits*s->dim+8)>>3);/*/8*/	/* remember flag bit */
			/* vector of column offsets; remember flag bit */
			long total2=(((_ilog(quantvals-1)*s->dim+8)>>3)+((s->q_bits+7)>>3));/*/8*/
			if (total1<=4 && total1<=total2)
			{
				...
				s->dec_type  = 1;
				s->dec_nodeb = _determine_node_bytes(s->used_entries, ((s->q_bits*s->dim+8)>>3));/*/8*/
				s->dec_leafw = _determine_leaf_words(s->dec_nodeb, ((s->q_bits*s->dim+8)>>3));/*/8*/
			...
			else
			{
				...
				s->dec_type  = 2;
				s->dec_nodeb = _determine_node_bytes(s->used_entries, ((_ilog(quantvals-1)*s->dim+8)>>3));/*/8*/
				s->dec_leafw = _determine_leaf_words(s->dec_nodeb, ((_ilog(quantvals-1)*s->dim+8)>>3));/*/8*/
				if (_make_decode_table(s,lengthlist,quantvals,opb,maptype))goto _errout;
			}
		}
		break;
	case 2:
		...
		if ( ((s->q_bits*s->dim+8)>>3) <= 4)/*/8*/		/* remember flag bit */
		{
			/* use dec_type 1: vector of packed values */
			s->dec_type  = 1;
			s->dec_nodeb = _determine_node_bytes(s->used_entries, ((s->q_bits*s->dim+8)>>3));/*/8*/
		...
		else
		{
			...
			s->dec_type  = 3;
			s->dec_nodeb = _determine_node_bytes(s->used_entries, (_ilog(s->used_entries-1)>>3)+1);/*/8*/
			s->dec_leafw = _determine_leaf_words(s->dec_nodeb, (_ilog(s->used_entries-1)>>3)+1);/*/8*/
			if (_make_decode_table(s,lengthlist,quantvals,opb,maptype))goto _errout;

			/* get the vals & pack them */
			s->q_pack=(((s->q_bits+7)>>3))*((s->dim));/*/8*/
...
_errout:
_eofout:
	vorbis_book2_clear(s);
	return (-1);
}

static inline ogg_uint32_t decode_packed_entry_number(
	codebook *book,
	oggpack_buffer *b)
{
...
long vorbis_book2_decode(codebook *book, oggpack_buffer *buffer)
{
	if (book->dec_type)return (-1);
	return decode_packed_entry_number(book,buffer);
}
...
int decode_map(codebook *s, oggpack_buffer *b, ogg_int32_t *v, int point)
{
	...
	case 2:
		/* packed vector of column offsets */
		{
			...
			for (i=0; i<s->dim; i++)
			{
				...
				else	{	v[i] = ((ogg_uint16_t *)(s->q_val))[entry&mask];}
			entry >>= s->q_pack;
			}
		}
		break;
	case 3:
		...
		{
		//	void *ptr;
		//	ptr = (void*)s->q_val + (entry * s->q_pack);
			unsigned char *ptr;
			ptr = (unsigned char*)s->q_val + (entry * s->q_pack);
			if (s->q_bits <= 8)
			{
				for (i=0; i < s->dim; i++)
...
//	src/libtremor_lowmem/codebook.c: In function 'decode_map':
//	src/libtremor_lowmem/codebook.c(762) : warning: pointer of type 'void *' used in arithmetic
	...
	/* we have the unpacked multiplicands; compute final vals */
	{
		...
		else			{	add = s->q_min << -add;}

		if (shiftM > 0) 	{	for (i=0; i<s->dim; i++)	{	v[i] = add + ((v[i] * s->q_del) >> shiftM);}}
		else				{	for (i=0; i<s->dim; i++)	{	v[i] = add + ((v[i] * s->q_del) << -shiftM);}}
		if (s->q_seq)		{	for (i=1; i<s->dim; i++)	{	v[i] += v[i-1];}}
	}
	return (0);
}

/* returns 0 on OK or -1 on eof *************************************/
long vorbis_book2_decodevs_add(
	codebook *book,
...
)
{
	if (book->used_entries > 0)
	{
		...
		for (j=0; j<step; j++)
		{
			if (decode_map(book, buffer, v, point)) 	{	return (-1);}
			int i;
			int o;
...
long vorbis_book2_decodevv_add(
	codebook *book,
	oggpack_buffer *b,
	int n,
	int point,
