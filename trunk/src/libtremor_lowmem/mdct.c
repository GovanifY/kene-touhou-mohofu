...
#if 0
//static unsigned char revbers4bit[16] = {0,8,4,12,2,10,6,14,1,9,5,13,3,11,7,15};
STIN int revbers12bit(unsigned int x)
{
	static const unsigned char revbers4bit[(0x10)] =
	{			// 1234
		0x00,	// 0000   0
		0x08,	// 1000   8
		0x04,	// 0100   4
	...
	return (revbers4bit[x>>8]|(revbers4bit[(x&0x0f0)>>4]<<4)|(((int)revbers4bit[x&0x00f])<<8));
}
#endif

/*static*/ STIN int revbers12bit(unsigned int x)
{
//	return (revbers4bit[x>>8]|(revbers4bit[(x&0x0f0)>>4]<<4)|(((int)revbers4bit[x&0x00f])<<8));
//	return (__builtin_allegrex_bitrev(x)>>20);/*(ng)*/
	return ((__builtin_allegrex_bitrev((unsigned)x)>>20)&0x0fff);/*(ok)*/
	//0x00000fffUL
	//0xfff00000UL
}
...
STIN void mdct_step7(DATA_TYPE *x, int n, unsigned int step)
{
	DATA_TYPE	*w0 	= x;
	DATA_TYPE	*w1 	= x+(n>>1);
...
STIN void mdct_step8(DATA_TYPE *x, int n, unsigned int step)
{
	LOOKUP_T *T;
	LOOKUP_T *V;
	DATA_TYPE *iX = x+(n>>1);
...
void mdct_shift_right(unsigned int n, DATA_TYPE *in, DATA_TYPE *right)
{
	unsigned int i;
	n >>= 2;
	in += 1;
