...
static ogg_uint32_t oggbyte_read4(oggbyte_buffer *b, int pos)
{
	...
	_positionB(b,pos);
	_positionF(b,pos);
	ret  = (b->ptr[pos-b->pos]	  );	_positionF(b,++pos);
	ret |= (b->ptr[pos-b->pos]<< 8);	_positionF(b,++pos);
	ret |= (b->ptr[pos-b->pos]<<16);	_positionF(b,++pos);
...
static ogg_int64_t oggbyte_read8(oggbyte_buffer *b,int pos)
{
	...
	// pack 8 bytes.
	_positionF(b,pos);
	ret = b->ptr[(pos)-(b->pos)];	// 1st bytes.
	for (i=6; i>=0; --i)			// 2nd 7bytes.
	{
	//	ret = ret<<8 | t[i];//original
		ret = ((ret<<8) | (t[i]));//???
	}
	return (ret);
}
...
static ogg_uint32_t _checksum(ogg_reference *or, int bytes)
{
	/*
		...
		See older code in CVS for dead run-time initialization code.
	*/
...
long ogg_sync_pageseek(ogg_sync_state *oy,ogg_page *og)
{
	...
	if (bytes<oy->headerbytes)goto sync_out; /* not enough for header + seg table */
	if (oy->bodybytes==0)
	{
		int i;
