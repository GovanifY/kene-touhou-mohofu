...
static void result_03_pad_check(void)
{
	if (0==psp_pad.pad_data_alter)/* さっき何も押されてなかった場合にキーチェック(原作準拠) */
	{
		...
		//	if (/*4*/7<show_player_num) 	{show_player_num=0;}	/* 最後まで見たら始めから見る */
			...
			cb.return_call_func 	= result_01_set_location;	/* また見るの場合の移動先を設定(反映するのは、まだ先) */
		...
		{
			cb.return_call_func 	= title_menu_start; 	/* タイトルメニューへ移動先を設定(反映するのは、まだ先) */
			cb.main_call_func		= result_04_set_slide_out;
		}
	}
//	yume_no_kiroku_draw();//要らない
...
global void yume_no_kiroku_start(void)
{
	...
	cb.main_call_func		= result_01_set_location;/* 動作先 */
