...
/* ---------- 下側床 ---------------------------------------- */
		x2 = (GAME_X_OFFSET+ 96) /*+ ((val1&0x7f)-64)*/;/*(固定端)*/
		x3 = (GAME_X_OFFSET+  0);
...
#define EH_Y_LOCATE_271 ((PSP_HEIGHT272-1)/*-1*/)
	...
	//
		...
		#else/*(裏側ポリゴンを描画しない設定の場合は、纏められる。)*/
		/* (カリング有効の場合、裏側ポリゴンが描かれない) */
	...
	// xxx向きが違うからダメ。	sceGuDrawArray(GU_TRIANGLE_STRIP, TEXTURE_FLAGS5650, (EH_R08V+EH_L08V+EH_24V), NULL, vertices); 		/* 上側、右/左/ふすま。下側床 */
}
