...
static void move_sakuya_baramaki2(SPRITE *src)
{
...
//
	...
	if (0x0ff < src->BOSS_DATA_05_move_jyumyou)/* 0x100未満は共通規格上撃たない */
	{
		...
		//	HATSUDAN_06_n_way					= (3);		/* [破壊] */				/* [3way x 2] */
...
void add_zako_sakuya_baramaki2(SPRITE *src)
{
	int i;
	for (i=0; i<(4); i++) /* 4つ */
	{
	...
	//
		...
		//	h->BOSS_DATA_05_move_jyumyou		= (0x03ff); 	/* 制限時間 (r32) */
			h->BOSS_DATA_05_move_jyumyou		= (0x02ff); 	/* 制限時間 (r33) */
		}
	}
}
