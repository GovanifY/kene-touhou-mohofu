...
//	u8 dummy_use_alpha; 					/* アルファ(画像透明度)使用可否 */
//	//u8 total_frames_dummy_resurved;	/* 現在ダミー */
//	u8 x_divide_frames_m1;				/*廃止*/	/* (横分割数-1) */
//	u8 y_divide_frames_m1;				/*廃止*/	/* (縦分割数-1) */
//
//	//u8 priority;						/* 描画プライオリティー(表示優先度) */
//	u8 aaa_dummy_anime_speed;			/*廃止*/	/* 現在ダミー アニメーション速度 */
//	u8 atari_hankei;					/* あたり判定円の半径(あたり判定の大きさ) */
//	u8 aaa_dummy_resurved;				/*廃止*/	/* 現在ダミー */
//} IMAGE_444_RESOURCE;
...
/*static*/ SPRITE *sprite_list444_head = NULL;	/* スプライトのリスト構造 */

//SPRITE sprite_pool[SPRITE_POOL_MAX];	/* スプライトのリスト構造 */

/*static*/ SPRITE *sprite_collision_check_444(SPRITE *tocheck, int type)
{
	SPRITE *s = sprite_list444_head;/* スプライト リストの先頭 から探す */
	while (NULL != s)
	{
		/* 1. 自分自身チェック */
...
/* 弾幕リストは分割する予定 */
//SPRITE *sp rite_collision_check_bullets(SPRITE *tocheck)
//{
//	return (sprite_collision_check_444(tocheck, SP_GROUP_BULLETS));/*とりあえず*/
//}
...
static void sprite_remove_one_444(SPRITE *src)
{
	...
	while (NULL != s)/* スプライト リストの終わりまで探す */
	{
		...
		if (s == src)/* 見つかった？ */
		{
			if (NULL == p)
			{
				sprite_list444_head = n;	/* スプライトリストの先頭を書き換える。 */
			}
			else
			{
				p->next = n;
...
void sprite_remove_all_444(int type)
{
	SPRITE *s = sprite_list444_head;/* スプライト リストの先頭 から探す */
	SPRITE *n = NULL;
	while (NULL != s)
...
void sprite_work_444(int type)
{
	SPRITE *s;
	SPRITE *n;
	s = sprite_list444_head;/* スプライト リストの先頭 から探す */
	while (NULL != s)
	{
		n = s->next;
		#if 0
	...
	if ( ST_WORK_MENU != (psp_loop&0xff00) )/* メニュー以外の場合、(ポーズメニューで時間が経つ場合に消えると困るので) */
	{
		...
		while (NULL != s)
		{
			...
			{
				sprite_remove_one_444(s);	/* 削除済みを消す */
			}
			s = n;
		}
	}
...
//	memset(obj, 0, sizeof(SPRITE));
	s			= sprite_list444_head;	/* スプライトリストの先頭。呼び出し */
	tail_obj	= NULL;
	/*---------------------------------------------------------
...
//		(s->priority < priority)	/* 表示プライオリティーを満たす位置まで来たら終わり */
	...
	{
		/* 先頭の場合は->nextが無いので */
		sprite_list444_head = obj;		/* スプライトリストの先頭を書き換える。 */
	}
...
//
//	#if 0/*SDL(左隅座標)*/
//	obj->w128				= (2<<8);//((surface->w/*/(x_divide_frames_m1)*/)<<7);/*弾の大きさの半分(とりあえず)*/
//	obj->h128				= (2<<8);//((surface->h/*/(y_divide_frames_m1)*/)<<7);/*弾の大きさの半分(とりあえず)*/
//	#else/*Gu(中心座標)*/
//	#endif
//
//	int total_frames;		total_frames	= image_resource_ptr->total_frames;
//	int anime_speed;		anime_speed 	= image_resource_ptr->anime_speed;
	int m_Hit256R;			m_Hit256R		= my_bullet_resource_atari_hankei[image_resource_num];//image_resource_ptr->atari_hankei;
//
