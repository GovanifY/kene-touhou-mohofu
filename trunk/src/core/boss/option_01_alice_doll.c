...
static void move_alice_doll_last_burrets(SPRITE *src)
{
	...
	br.BULLET_REGIST_angle1024			= (src->rotationCCW1024);			/* 角度 */
	br.BULLET_REGIST_div_angle1024		= (int)(1024/23);					/* 分割角度 (1024/27) (1024/24) */
	br.BULLET_REGIST_bullet_obj_type	= BULLET_UROKO14_00_AOI+(difficulty);			/* [青鱗弾] */
	...
	br.BULLET_REGIST_bullet_obj_type	= BULLET_UROKO14_00_AOI+(difficulty);			/* [青鱗弾] */
	br.BULLET_REGIST_n_way				= (1);								/* [7way] [8way] */
	...
	for (i=(0); i<(7); i += (1) )	/* 弾数 */
	{
		...
		br.BULLET_REGIST_speed256			= (ccc[i][CCC_00_SPEED256]);									/* 弾速 */
		br.BULLET_REGIST_angle65536			= (first_angle_65536+ccc[i][CCC_01_ADD_ANGLE65536]);			/* 角度 */
...
static void move_alice_doll_all(SPRITE *src)
{
	...
	else/* 移動処理 */
	{
		...
		//	bbb_rotationCCW1024 = src->rotationCCW1024-(1024/8);/* -(1024/8) == -45/360度傾ける */
			...
			src->radius++;
			src->cx256 = src->sx256 + ((sin1024((bbb_rotationCCW1024))*src->radius));/*fps_factor*/
			src->cy256 = src->sy256 + ((cos1024((bbb_rotationCCW1024))*src->radius));/*fps_factor*/
		}
	}
}
...
//	for (i_angle1024=(0); i_angle1024<(1024); i_angle1024 += (1024/7)/*add_angle*/) 	/* 弾数 */
	...
	//
			h->time_out 			= (60);/* 1[秒](60[frame])後に、再分裂 */
			{
				/* 初期位置 */
