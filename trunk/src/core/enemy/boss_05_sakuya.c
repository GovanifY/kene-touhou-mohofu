...
//++	((PLAYER_DATA *)player->data)->bo ssmode=B04_CHANGE;
	...
	data->wait3 		= 0;//???
	data->time_out		= 60*60;	// 60 [秒] ???	4096/*2500*/	/*3000*/;//???
}
...
static void callback_enemy_boss05_hitbyweapon(SPRITE *s, SPRITE *t/*, int angle*/)
{
	...
	if (data->enemy_base.health < 1)/*全員倒した？*/
	{
	...
	//	((PLAYER_DATA *)player->data)->bo ssmode=B02_BOSS_DESTROY;
		...
		//
		explosion_add_circle(s, 1); 	/* B02_BOSS_DESTROY が必要 */
		s->type=SP_DELETE;
	}
}
...
//------------------------------------------
	...
	*/
	case 5: 	//星型を描くよ
		...
		{
			d->angle512=atan_512(t256(48)-s->y256-((s->h)<<7), t256(190)-s->x256-((s->w)<<7));/*星型線1*/
			d->state01=6;
			d->state02=0x0;
		}
		break;
	case 6:
		...
		{
			d->state01=7;
			d->angle512=atan_512(t256(217)-s->y256-((s->h)<<7), t256(248)-s->x256-((s->w)<<7));/*星型線2*/
			d->wait=50;
	...
	case 9:
		...
		{
			d->state01=10;
			d->angle512=atan_512(t256(113)-s->y256-((s->h)<<7), t256(100)-s->x256-((s->w)<<7));/*星型線3*/
			d->wait=50;
			d->state02=0x0;
		}
		else
	...
	case 12:
		...
		{
			d->state01=13;
			d->angle512=atan_512(t256(113)-s->y256-((s->h)<<7), t256(280)-s->x256-((s->w)<<7));/*星型線4*/
			d->wait=50;
			d->state02=0x0;
		}
		else
	...
	case 15:
		...
		{
			d->state01=16;
			d->angle512=atan_512(t256(217)-s->y256-((s->h)<<7), t256(138)-s->x256-((s->w)<<7));/*星型線5*/
			d->wait=50;
			d->state02=0x0;
		}
		else
...
static void enemy_boss05_move(SPRITE *s)
{
	...
	case SAKUYA_01_KEITAI/*0*/: // 第一形態: 左右に動いて小弾撃ち
		...
		case 0: 	/* 下がる */
			...
			data->time_out		= 60*60;	// 60 [秒] ??? /*時間制限カウント無効化*/
			break;
		/*完全に姿を現す*/
		case 10:	/* */
			data->time_out		= 60*60;	// 60 [秒] ??? /*時間制限カウント無効化*/
	...
	case SAKUYA_04_KEITAI_B/*4*/:	// 第四形態: (黄色マスカット弾)
		...
		case 0:
			if (data->wait1<0)	// 待機時間が終わったら
			{
				data->state2=1;
//++				pd->bo ssmode=B00_NONE/*B01_BA TTLE*/;
//???			data->time_out += (20*60);			// + 20 [秒] ???	/* 16.666[秒] 1000 ???*/
				s->anim_frame=15;
			}
			else
			{
...
//				if (2==select_player)	{	data->enemy_base.health=1; }
	...
	data->enemy_base.score = adjust_score_by_difficulty(score(5000000));	/* 500万 */
...
//	data->time_out		= (20*60);		/*1000*/	// + 20 [秒] ???	/* 16.666[秒] 1000 ???*/
	data->time_out		= 60*60;	// 60 [秒] ???
