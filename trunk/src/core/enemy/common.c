...
void callback_hit_zako(SPRITE *src/*敵自体*/, SPRITE *tama/*自弾*/)
{
	...
	bakuhatsu_add_type_ddd(dummy_obj/*t->x256,t->y256*/,/*0,*/BAKUHATSU_MOVE12/*BAKUHATSU_MINI00*/);/*先に実行した方が速い*/
//
	{
	...
	//	WEAPON_BASE *w		= (WEAPON_BASE *)tama->data;
		...
		if (0 >= /*data->*/src->base_health)			/* ０か負値なら、倒した。 */
		{
			...
			bonus_info_any_score_nodel(src, ((/*data->*/src->base_score)*(/*pd->now_stage*/player_now_stage/*level*/)) );/*自動消去へ仕様変更*/src->type = SP_DELETE;
			#if (0==USE_DESIGN_TRACK)
...
/*static*/ void callback_hit_boss(SPRITE *src/*敵自体*/, SPRITE *tama/*自弾*/)/*, int angle*/
{
	dummy_obj->x256 = tama->x256;
	dummy_obj->y256 = tama->y256;
	bakuhatsu_add_type_ddd(dummy_obj/*tama->x256,tama->y256*/,/*0,*/BAKUHATSU_MOVE12/*BAKUHATSU_MINI00*/);/*先に実行した方が速い*/
//
	{/* BOSS_BASE */
...
//
	...
	if ( 0x00 == (data->time_out&0x0f) )/*0x07*/
	{
		//static void enemy_homing_update(SPRITE *src)
		{
			...
			#if 0
			case ST01:
				dummy_obj->x256 = data->p1_x256;	/* 発弾位置の offset 用 */
				dummy_obj->y256 = data->p1_y256;	/* 発弾位置の offset 用 */
				...
				break;
			case ST02:
				dummy_obj->x256 = data->p2_x256;	/* 発弾位置の offset 用 */
				dummy_obj->y256 = data->p2_y256;	/* 発弾位置の offset 用 */
				src->m_angleCCW512/*data->aaa_angle512*/ = angle_jikinerai512(dummy_obj,s);
				/* 十分ちかづいたら */
				if (1024 >(dummy_obj->y256-src->y256)+(dummy_obj->x256-src->x256) )
				{
