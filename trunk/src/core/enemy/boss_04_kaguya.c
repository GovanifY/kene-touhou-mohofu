...
//
	...
	{	int b_health_alter_low1024;
		b_health_alter_low1024 = (data->enemy_base.health & (1024-1));/* 直前の下位1024 */
		data->enemy_base.health -= w->strength;/* ボス攻撃して体力減らす */
		if (b_health_alter_low1024 < (data->enemy_base.health & (1024-1)))/* 形態変更したら */
		{
			if (i==1)	// 輝夜かどうかチェック
			{
	...
	if ((data->enemy_base.health<=15) &&
		(data->health_flag==0))
	{
		data->health_flag=1;
		explosion_add_type(sb03[i]->x256+((sb03[i]->w)<<7),sb03[i]->y256+((sb03[i]->h)<<7),/*0,*/EXPLOSION_FIRE08);
	}
	if (data->enemy_base.health<=0)/* 該当者の体力切れなら */
	{
		b01flags			|= (FLG_DOLL0<<i);/*倒した人をONにする*/
		explosion_add_type(sb03[i]->x256+((sb03[i]->w)<<7),sb03[i]->y256+((sb03[i]->h)<<7),/*0,*/EXPLOSION_ZAKO04/*EXPLOSION_FIRE08*/);
...
//	if (j==0)/*全員倒した？*/
	...
	//	((PLAYER_DATA *)player->data)->bo ssmode=B02_BOSS_DESTROY;
		...
		//	explosion_add_type(sb03[i]->x256+((sb03[i]->w)<<7),sb03[i]->y256+((sb03[i]->h)<<7),/*(ra_nd()&(16-1)/*%20*/),*/EXPLOSION_FIRE08);
			explosion_add_circle(sb03[i], 1);	/* B02_BOSS_DESTROY が必要 */
...
//
	sb03[0]->x256 = x256;
	sb03[1]->x256 = x256 + ((sb03[0]->w)<<8);
	sb03[2]->x256 = x256 + ((sb03[0]->w)<<8) + ((sb03[1]->w)<<8);
//
	sb03[0]->y256 = y256;
	sb03[1]->y256 = y256;
	sb03[2]->y256 = y256;
//
	sb03[3]->x256 = x256;
	sb03[4]->x256 = x256 + ((sb03[3]->w)<<8);
	sb03[5]->x256 = x256 + ((sb03[3]->w)<<8) + ((sb03[4]->w)<<8);
//
	sb03[3]->y256 = y256 + ((sb03[0]->h)<<8);
	sb03[4]->y256 = y256 + ((sb03[1]->h)<<8);
	sb03[5]->y256 = y256 + ((sb03[2]->h)<<8);
}
...
static void center_shot(BOSS03_DATA *data)
{
	...
	if (1==b01level/*0==data->level*/)
	{
		...
		kakudo = (127-16);
		kakudo += ((ra_nd())&(32-1));
		switch ((data->enemy_base.health)&(0xfc00) )
		{
		case (3<<10):/*not_break;*/
...
/*完全に姿を現す*/
	...
	if (1==b01level/*0==data->level*/)
	{
		if (data->enemy_base.health <= 1000+1024)
		//switch ( (data->enemy_base.health)&0xfc00 )
		{
		//case (0<<10):
			if (firewait3 < 0)
			{
				...
				if (data->enemy_base.health>200+1024)
				{	firewait3=data->enemy_base.health/4;}
				else
				{	firewait3=50;}
			}
			else
...
//	sb03[0] = sprite_add_file 0("boss/boss04_0.png"/*"boss03-lo.png"*/, 2, PRIORITY_03_ENEMY, 0);
//	sb03[1] = sprite_add_file 0("boss/boss04_1.png"/*"boss03-mo.png"*/, 2, PRIORITY_03_ENEMY, 0);
//	sb03[2] = sprite_add_file 0("boss/boss04_2.png"/*"boss03-ro.png"*/, 2, PRIORITY_03_ENEMY, 0);
//	sb03[3] = sprite_add_file 0("boss/boss04_3.png"/*"boss03-lu.png"*/, 2, PRIORITY_03_ENEMY, 0);
//	sb03[4] = sprite_add_file 0("boss/boss04_4.png"/*"boss03-mu.png"*/, 2, PRIORITY_03_ENEMY, 0);
//	sb03[5] = sprite_add_file 0("boss/boss04_5.png"/*"boss03-ru.png"*/, 2, PRIORITY_03_ENEMY, 0);
	...
	for (i=0;i<6;i++)
	{
		...
		if (lv==0)		// [***090114		追加
		{
			if (i==1)/* 輝夜本人 */
			{
				data->enemy_base.health 	= (difficulty<<10/*[x1024]*/)+2047+1024;
			//	data->enemy_base.score		= score(3000)+score(2000)*difficulty;
				data->enemy_base.score		= adjust_score_by_difficulty(score(2000000));	/* 200万 (計400万==(200万)+(5x40万)) */
			}
			else/* 小物類 */
			{
				data->enemy_base.health 	= (difficulty<<8/*[x256]*/)+ 200+1024;
			//	data->enemy_base.score		= score(600)*(difficulty+1);
				data->enemy_base.score		= adjust_score_by_difficulty(score( 400000));	/* 40万 */
			}
		}
		else/* 全部一緒 */
		{
			data->enemy_base.health 	= (difficulty<<6/*[x64]*/)+ 200+1024;
		//	data->enemy_base.score		= score(500)*(difficulty+1);
			data->enemy_base.score		= adjust_score_by_difficulty(score( 500000));	/* 50万 (計300万==6x50万) */
		}
		data->health_flag		= 0;
		if (i==1)
		{
