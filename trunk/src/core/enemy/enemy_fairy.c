...
static void enemy_fairy_move(SPRITE *s)
{
	...
	if (difficulty)
	{
		...
		case 4: 	if (s->y>270)		{	enemy_n_way_bullet(s, "kugel.png", 0, 8, difficulty*5, atan2(player->y+player->h/2-s->y,player->x-player->w/2-s->x));
																					d->state=5; 	}	break;
		case 5: 	if (s->y>HEIGHT)	{	s->type=-1; 											}	break;
		}
	}
}
