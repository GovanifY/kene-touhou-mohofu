...
static void enemy_stop_bullet2_move(SPRITE *s)
{
	...
	if ((s->x<-50)||(s->x>WIDTH2+60)||
		(s->y>HEIGHT+60)||(s->y<-50)
	//	||(s->alpha<0)
	)
	{
		s->type=-1;
...
void enemy_n_way_bullet(SPRITE *s, char *filename, int frame, int n, double speed, double angle)
{
	...
	BULLET_DATA *data;
	int i;
	double i_angle=angle-(n+2)*((M_PI*2)/48);	//π/24 /*0.13089969389957471827*/
...
void explosion_add(int x, int y, double wait, int type)
{
	...
	if (EXPLOSION_TYPE04==type)	{	type = (rand()%3+1);	}/* あちこちにあると遅い */
	switch (type)
	{
	case 0: 	ex=sprite_add_file("ex.png",29,PR_BONUS);		d->framewait=2; 	break;
	case 1: 	ex=sprite_add_file("tr_blue.png",6,PR_BONUS);	d->framewait=1; 	break;
	case 2: 	ex=sprite_add_file("tr_red.png",6,PR_BONUS);	d->framewait=1; 	break;
	case 3: 	ex=sprite_add_file("tr_green.png",6,PR_BONUS);	d->framewait=1; 	break;
