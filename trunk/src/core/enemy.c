...
void bullet_way(SPRITE *s ,int way, dou ble angle512, dou ble angle2, dou ble spd)
{
	...
	{
		SPRITE *h;
		h					= sprite_add_file 0("kugel.png",1,PRIORITY_05_BULLETS/ * PRIORITY_03_ENEMY * /,0);
		h->type 			= SP_BULLET;
		h->flags			|= (SP_FLAG_VISIBLE|SP_FLAG_COLISION_CHECK|SP_FLAG_TIME_OVER);
		h->callback_mover	= move_bullet_angle512;
		h->anim_frame		= 0;
		h->x256 			= s->x256+((s->w-h->w)<<7);
		h->y256 			= s->y256+((s->h-h->h)<<7);
		BULLET_DATA *data;
...
/* この方式では親が先に死んで子供が行方不明になる場合がある。(サイズが 親==子なら問題ない)
	...
	既に再利用されている場合がある。  */
//	s->x256 = *(data->sx256p)+((data->sw-s->w)<<7);
//	s->y256 = *(data->sy256p)+((data->sh-s->h)<<7);
...
{
	ko->x256=s->x256+((s->w-ko->w)<<7);/**/
	ko->y256=s->y256+((s->h-ko->h)<<7);/**/
...
static int angle_jikinerai512(SPRITE *p, SPRITE *t)
{
	...
	//	return (atan_512(p->y256-t->y256+((p->h-t->h)<<7),p->x256-t->x256-((p->w+t->w)<<7)));/*???(original)*/	/* 奇数弾の場合に自機狙い(?) なんか左にずれる */
		return (atan_512(p->y256-t->y256+((p->h-t->h)<<7),p->x256-t->x256-((p->w-t->w)<<7)));/**/				/* 奇数弾の場合に自機狙い */
	//	return (atan_512(p->y256-t->y256+((p->h-t->h)<<7),p->x256-t->x256+((p->w-t->w)<<7)));/**/				/* 偶数弾の場合に自機狙い */
	//	return (atan_512(p->y256-t->y256+((p->h 	)<<7),p->x256-t->x256-((p->w	 )<<7)));
	//	return (atan_512(p->y256-t->y256-((p->h 	)<<7),p->x256-t->x256-((p->w	 )<<7)));
	#endif
}
/*---------------------------------------------------------
...
static void enemy_follow_knife_move(SPRITE *s)
{
	...
	else
	{
		...
		if (d->speed256 <= t256(0) )
		{
			d->angle512=atan_512(player->y256-s->y256+((player->h)<<7)+(d->height*256),player->x256-s->x256-((player->w)<<7));
			d->target=1;
...
static void enemy_evenl_knife_move(SPRITE *s)
{
	...
	//	s->anim_frame	= /*17-*/(deg_512_to_360(d->angle512-(int)(512*1/4))/20)%16;
		...
		s->x256 		= d->x256-((cos512((d->angle512))*d->length256)>>8)-((s->w)<<7);
		s->y256 		= d->y256-((sin512((d->angle512))*d->length256)>>8)-((s->h)<<7);
...
void bullet_create_hari_dan180(SPRITE *s, int speed256, int angle512, int x_offset256, int y_offset256 )
{
	...
	{
		angle512	= angle_jikinerai512(player,s);
	}
//
	SPRITE *h;
//	h					= sprite_add_file 0(/*"bs hoot.png"*/"tama/bullet_beam16.png",16/*1*/,PRIORITY_05_BULLETS/*PRIORITY_03_ENEMY_WEAPON*/ /*PRIORITY_03_ENEMY*/, 0);
	...
	h->x256 			= s->x256+((s->w-h->w)<<7) + x_offset256;
	h->y256 			= s->y256+((s->h-h->h)<<7) + y_offset256;
...
{		/*丸弾８(赤ＲＧＢ緑若黄青)*/
	SPRITE *h;
	h					= NULL;
//	h					= sprite_add_file 0("tama/bs hoot.png", 1, PRIORITY_05_BULLETS/*PRIORITY_03_ENEMY*/, 0);
//	h					= sprite_add_file 0("tama/bullet_maru8.png", 8, PRIORITY_05_BULLETS/*PRIORITY_03_ENEMY*/, 0);
	h					= sprite_add_res(BASE_TAMA_BULLET_MARU16_PNG);
	h->type 			= SP_BULLET/*SP_LASER*/;
	h->callback_mover	= move_bullet_vector/*move_bullet_angle512*/;/*enemy_laser_move*/
	h->flags			|= (SP_FLAG_VISIBLE|SP_FLAG_COLISION_CHECK|SP_FLAG_TIME_OVER);
...
{	/*丸弾８(赤ＲＧＢ緑若黄青)*/
	SPRITE *h;
//	h					= sprite_add_file 0("tama/sp ell_bullet_r.png", 1, PRIORITY_05_BULLETS/*PRIORITY_03_ENEMY*/, 0);
	h					= sprite_add_res(BASE_TAMA_BULLET_MARU16_PNG);
	h->type 			= SP_BULLET/*SP_LASER*/;
...
{
	SPRITE *h;
//	h					= sprite_add_file 0("tama/kugel2.png", 1, /*PRIORITY_05_BULLETS*/PRIORITY_05_BULLETS/*PRIORITY_03_ENEMY*/, 0);
//	h					= sprite_add_res(BASE_TAMA_KUGEL_MINI2_PNG);
	h					= sprite_add_res(BASE_TAMA_BULLET_MARU16_PNG);
	h->type 			= SP_BULLET;
...
//	h					= sprite_add_file 0("tama/bs hoot2.png",3,/*PRIORITY_05_BULLETS*/PRIORITY_05_BULLETS/*PRIORITY_03_ENEMY*/,0);/*緑黄弾*/
...
{
	SPRITE *h;			/*丸弾８(赤ＲＧＢ緑若黄青)*/
//	h					= sprite_add_file 0("tama/bs hoot.png",1,/*PRIORITY_05_BULLETS*/PRIORITY_05_BULLETS/*PRIORITY_03_ENEMY*/,0);/*青弾*/
...
{
	SPRITE *h;			/*丸弾８(赤ＲＧＢ緑若黄青)*/
//	h					= sprite_add_file 0("tama/bs hoot2.png", 3, PRIORITY_03_ENEMY, 0);
//	h					= sprite_add_file 0("tama/bullet_maru8.png", 8, PRIORITY_03_ENEMY, 0);
...
{		/*丸弾８(赤ＲＧＢ緑若黄青)*/
	SPRITE *h;
//	h					= sprite_add_file 0("tama/kugel.png", 1, PRIORITY_03_ENEMY, 0);
//	h					= sprite_add_res(BASE_TAMA_KUGEL_PNG);	h->priority 	= PRIORITY_03_ENEMY;
	h					= sprite_add_res(BASE_TAMA_BULLET_MARU16_PNG);	h->priority 	= PRIORITY_03_ENEMY;
	h->type 			= SP_BULLET;
...
{
	SPRITE *h;
//	h					= sprite_add_file 0("tama/knife.png",1,/*PRIORITY_05_BULLETS*/PRIORITY_05_BULLETS/*PRIORITY_03_ENEMY*/,1);/*垂直降下ナイフ(赤)*/
...
void enemy_knife_create(SPRITE *s, int speed256, int angle512, int anime16_pattern)
{
	...
	h					= sprite_add_file 0(filename,1,/*PRIORITY_05_BULLETS*/PRIORITY_05_BULLETS/*PRIORITY_03_ENEMY*/,1);/*全方向ナイフ(青)*/
	h->type 			= SP_BULLET;
	h->callback_mover	= move_bullet_vector/*move_bullet_angle512*/;
	h->flags			|= (SP_FLAG_VISIBLE|SP_FLAG_COLISION_CHECK|SP_FLAG_TIME_OVER);
	h->anim_frame		= /*anim%18*/0;
	#else
//	h					= sprite_add_file 0("tama/knife_core.png",18,/*PRIORITY_05_BULLETS*/PRIORITY_05_BULLETS/*PRIORITY_03_ENEMY*/,1);/*全方向ナイフ(青)*/
...
//	if (0==r_or_l)
	...
	data->x256			= /*s->x256+((s->w)<<7)*/h->x256+((cos512((tmp512))*length256)>>8);
	data->y256			= /*s->y256+((s->h)<<7)*/h->y256+((sin512((tmp512))*length256)>>8);
...
void bullet_create_n_way_dan_type(SPRITE *s, int speed256, int angle512, int bu_type,/*char *filename, int frame,*/ int n)
{
	...
	{
		angle512=angle_jikinerai512(player,s);/*???(original)*/ 	/* 奇数弾の場合に自機狙い */
	}
	int i_angle512;
...
//
	...
	{
		SPRITE *h;
	//	h					= sprite_add_file 0("tama/bullet_ming32.png"/*"kugel.png"filename*/, 32/*(0)frame+1*/, PRIORITY_05_BULLETS/*PRIORITY_03_ENEMY*/,0);
...
//
	SPRITE *h;
//	h					= sprite_add_file 0("tama/bigkugel2.png",1,/*PRIORITY_05_BULLETS*/PRIORITY_03_ENEMY/*PRIORITY_03_ENEMY*/,0);/* 黒玉(輪)  あたり判定部分*/
...
//	h->flags			|= (/*表示なしSP_FLAG_VISIBLE|*/SP_FLAG_COLISION_CHECK);
	...
	h->anim_frame		= 0;
	h->x256 			= s->x256-((h->w)<<7)+xoffs256;
	h->y256 			= s->y256-((h->h)<<7)+yoffs256;
	#if 1
	SPRITE *h2;
//	h2					= sprite_add_file 0("tama/bigkugel1.png",1,PRIORITY_05_BULLETS/*P R_TMP*/,1);/* 大玉(青) 表示部分 */
//	h2					= sprite_add_res(BASE_TAMA_OODAMA_01_PNG);/* 大玉(青) 表示部分 */
	...
	h2->x256			= s->x256-((h2->w)<<7)+xoffs256;
	h2->y256			= s->y256-((h2->h)<<7)+yoffs256;
	#endif
...
//
	SPRITE *h;
//	h					= sprite_add_file 0(/*"new_bigkugel.png"*/"bigkugel1.png"/*"bigkugel1.png"*/,1,/*PRIORITY_05_BULLETS*/PRIORITY_03_ENEMY/*PRIORITY_03_ENEMY*/,0);/* 大玉(青) あたり判定部分*/
...
//	h->flags			|= (/*表示なしSP_FLAG_VISIBLE|*/SP_FLAG_COLISION_CHECK);
	...
	#if 1
	SPRITE *h2;
//	h2					= sprite_add_file 0("tama/new_bigkugel.png",1,PRIORITY_05_BULLETS/*P R_TMP*/,1);/* 大玉(赤) 表示部分*/	/*"new_bigkugel.png"*/	/*"bigkugel1.png"*/
//	h2					= sprite_add_res(BASE_TAMA_OODAMA_02_PNG);/* 大玉(赤) 表示部分*/	/*"new_bigkugel.png"*/	/*"bigkugel1.png"*/
	...
	h2->x256			= s->x256-((h2->w)<<7);
	h2->y256			= s->y256-((h2->h)<<7);
	#endif
	OODAMA_OYA_DATA *data;
	data				= mmalloc(sizeof(OODAMA_OYA_DATA));
	h->data 			= data;
...
//	if (EXPLOSION_MINI00==type) {	type = (ra_nd()%3+1);	}/* あちこちにあると遅い */
	...
	{
//	case 0: break;/* 小爆発 */
//	case 1: 	ex = sprite_add_file 0("tr_blue.png",  6, PRIORITY_04_ITEM, 0);	data->anime_speed=1;	break;
//	case 2: 	ex = sprite_add_file 0("tr_red.png",   6, PRIORITY_04_ITEM, 0);	data->anime_speed=1;	break;
//	case 3: 	ex = sprite_add_file 0("tr_green.png", 6, PRIORITY_04_ITEM, 0);	data->anime_speed=1;	break;
...
//
//	case 4: break;/* ザコ消滅爆発 */
//	case 5: 	ex = sprite_add_file 0("bakuha05.png", 5, PRIORITY_04_ITEM, 1);	data->anime_speed=/*8*/(3)/*1*/; ex->alpha = 150;	break;
//	case 6: 	ex = sprite_add_file 0("bakuha06.png", 5, PRIORITY_04_ITEM, 1);	data->anime_speed=/*8*/(3)/*1*/; ex->alpha = 150;	break;
//	case 7: 	ex = sprite_add_file 0("bakuha07.png", 5, PRIORITY_04_ITEM, 1);	data->anime_speed=/*8*/(3)/*1*/; ex->alpha = 150;	break;
...
//		/* 火炎爆発 */
//	case 8: 	ex = sprite_add_file 0("ex.png", 29, PRIORITY_04_ITEM, 0);		data->anime_speed=3/*2*/;	break;
