...
//	if ((0x10)==((src->boss_spell_timer)&0x1f))/* (16回に1回)(128なら計8回) */
	if ((0x40)==((src->boss_spell_timer)&0x7f))/* (16回に1回)(128なら計8回) */
	{
		#if (1)
	...
	if ((0x08)==((src->boss_spell_timer)&0x0f))/* (16回に1回)(8回毎に発弾) */
	{
...
//
	...
	//	const unsigned int aaa = (((~(src->boss_spell_timer))>>3)&0x1f);/* テキトーに大きくしてみる。 */
		const unsigned int aaa = (((~(src->boss_spell_timer))>>3)&0x1f);/* テキトーに大きくしてみる。 */
		unsigned int jj;
...
//	if ((0x10)==((src->boss_spell_timer)&0x1f))/* (16回に1回)(128なら計8回) */
	...
	if ((0x04)==((src->boss_spell_timer)&0x07))/* (8回に1回)(8回毎に発弾) */
	{
		static int aaa_angle65536;
		static int bbb_angle65536;
		br.BULLET_REGIST_03_tama_data				= (TAMA_DATA_8000_NON_TILT);/* (r33-)非傾き弾 */
...
{
	if ((32*20)-1==((src->boss_spell_timer) ))/* 約1.0[秒](==64flame)弾幕展開 */
	{
		//	voice_play(VOICE15_BOSS_KOUGEKI_01, TRACK04_TEKIDAN);
			bullet_play_04_auto(VOICE15_BOSS_KOUGEKI_01);
		/* 弾生成 */
		...
		//	br.BULLET_REGIST_02_angle65536				= (65536/16);			/* 向き */
			br.BULLET_REGIST_02_angle65536				= (65536/2);							/* [破壊] */	/* 真上向き */
			br.BULLET_REGIST_06_n_way					= (32); 								/* [破壊] */	/* [32way] */	/* 32方向弾 */
			br.BULLET_REGIST_07_div_angle65536			= /*(1792);*/((65536-(65536/8))/32);	/* [破壊] */	/* 分割角度(弾がこない範囲を除き32分割) */	/* (ra_nd16>>4) == 乱数(0-4095) */
			tama_system_regist_n_way();/* (r33-) */
		}
	}
	else
	if (((32*18)-HATUDAN_FRAME64)==((src->boss_spell_timer) ))/* 約0.33[秒](==20flame)停止 */
	{
		/* ここでは効果音なし */
		/* 全弾停止 */
	...
	if (((32*18)-HATUDAN_FRAME64-20)==((src->boss_spell_timer) ))/* 止まってから20flame後 */ /*(32*17)*/
	{
...
static void spell_create_26_aya_saifu(SPRITE *src)
{
	...
	if ((SPELL_TIME_9999-1)==((src->boss_spell_timer) ))/* 初期化 */
	{
		zzz = (64);/* 64から始める。 */
		ggg = 0;
	}
...
//		br.BULLET_REGIST_01_speed_offset			= t256(0);/*(テスト)*/
		...
		br.BULLET_REGIST_06_n_way					= (16*5);			/*(6)*/ 	/* 発弾数 */
...
//		br.BULLET_REGIST_02_angle65536				= ((65536/2));				/* 発射中心角度 / 特殊機能(自機狙い/他) */
	...
	//	br.BULLET_REGIST_06_n_way					= (17); 			/*(6)*/ 	/* 発弾数 */
		br.BULLET_REGIST_06_n_way					= (16); 			/*(8)*/ 	/* 発弾数 */
...
{
	if (0==((src->boss_spell_timer)&0x03))	// 4カウントに1回上に8way弾を撃つ
	{
			br.BULLET_REGIST_03_VECTOR_regist_type			= LEGACY_REGIST_TYPE_00_HANERU;
			br.BULLET_REGIST_04_bullet_obj_type 			= (BULLET_KOME_03_AOI); 	/* [青白米弾] */	/* 弾グラ */
			#if (1==USE_HAZUMI)
...
//			br.BULLET_REGIST_07_VECTOR_div_angle1024		= (int)(1024/64);
		...
		//	b05_fire_flags &= (~(doll_data->identity_bit));/* off */
			...
			br.BULLET_REGIST_02_VECTOR_angle1024			= (1024/4)+(1024/8)+(j<<5)+((src->boss_spell_timer>>2)&0x3f);
