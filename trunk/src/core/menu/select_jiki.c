...
//	/*static*/ char str_pa ss_word[32/*20*/];
	...
	if (0==psp_pad.pad_data_alter)/* さっき何も押されてなかった場合にキーチェック(原作準拠) */
	{
		...
		//
			if (psp_pad.pad_data & (PSP_KEY_LEFT|PSP_KEY_RIGHT)/*左か右の両方*/  ) /* 左右ボタン入力 */
			{
				...
				main_call_func = player_select_03_load; 	/* [load] */	/*(戻る)*/
			}
...
static void player_select_04_move_draw(void)/* [描画1] */
{
	...
	{
		sp_scale256 = (0);
		main_call_func = player_select_05_select;/* [選択] */
	}
...
static void player_select_03_load(void)/* [load] */
{
	...
	main_call_func = player_select_04_move_draw;
}
...
---------------------------------------------------------*/
static void rank_select_00_load(void);/*(宣言が必要)*/
	...
	if (0==psp_pad.pad_data_alter)/* さっき何も押されてなかった場合にキーチェック(原作準拠) */
	{
		...
		//
			if (psp_pad.pad_data & (PSP_KEY_UP|PSP_KEY_DOWN)/*上か下の両方*/ ) /* 上下ボタン入力 */
			{
				...
				voice_play(VOICE02_MENU_SELECT, TRACK01_EXPLODE);
				main_call_func = rank_select_00_load;	/* [load] */	/*(戻る)*/
			}
			if (psp_pad.pad_data & PSP_KEY_SHOT_OK)
			{
				cg.msg_time = (0);/* 必要 */
				...
				main_call_func = player_select_03_load;
			}
			else
			if (psp_pad.pad_data & PSP_KEY_BOMB_CANCEL)
			{
...
static void rank_select_01_move_draw(void)/* [描画1] */
{
	...
	{
		sp_scale256 = (0);
		main_call_func = rank_select_02_select;/* [選択] */
	}
//	SDL_SetAlpha(player_select_bg0_surface, SDL_SRCALPHA, (255-sp_scale256)/*bg_alpha_bbb*/);
//	SDL_BlitSurface(player_select_bg0_surface,NULL,sdl_screen[SDL_00_VIEW_SCREEN],NULL);
	psp_pop_screen();
...
---------------------------------------------------------*/
static void rank_select_00_load(void)	/* [load] */
{
	...
	main_call_func = rank_select_01_move_draw;
}
...
//	psp_clear_screen();
	...
	if (0==psp_pad.pad_data_alter)/* さっき何も押されてなかった場合 */
	{
		...
		main_call_func = rank_select_00_load;
	}
}
