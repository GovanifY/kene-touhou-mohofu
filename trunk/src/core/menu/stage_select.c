...
static void stage_select_menu_work(void)
{
	...
	//	static int www=10;
		...
		{
		case MENU_STATE_00_FADE_IN_MENU:	/* Increasing aaa.menu_brite, fade menu in */
		...
		case MENU_STATE_01_WORK_MENU: /* fadein complete */
			...
			break;
		case MENU_STATE_02_FADE_OUT:	/* selection done, decreasing aaa.menu_brite, fade menu out */
			aaa.menu_brite -= FPS_MENU_FACTOR8;
			if (0 >= (aaa.menu_brite) )
			{
				aaa.menu_brite	= 0;
...
//		int jjj = ((aaa.menu_item_my_obj[i]->SDL_h128)>>7)+5;
		int jjj = 5+16;
		aaa.menu_item_my_obj[i].MENU_DATA_i1 	= (PSP_HEIGHT272/2+40) -((jjj)*((aaa.max_items)>>1)) +(i*(jjj));
		aaa.menu_item_my_obj[i].MENU_DATA_i2 	= 0;
		aaa.menu_item_my_obj[i].MENU_DATA_i3 	= 0;
	}
//
...
//	fade_out_flag	= (0)/*set_fade_out_flag*/;/*構造的問題*/
	...
	*/
	...
	main_call_func = stage_select_menu_work;
}
