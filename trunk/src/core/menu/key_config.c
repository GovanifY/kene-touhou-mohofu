...
static void key_config_state_03_fade_init(void)
{
	...
	bg_alpha_aaa		= (0);
	main_call_func = key_config_state_04_fade_out;
}
		...
		else	/* 上下以外の通常処理(斜め入力を排除する為、上下の入力があった場合はそれ以外の処理をしない) */
		{
			...
			if (KEY_CONFIG_MENU_13_QUIT == menu_cursor1)	// 項目[ QUIT ] を選んでいる場合
			{
				if (psp_pad.pad_data & PSP_KEY_BOMB_CANCEL) // キャンセルボタン入力
				{
					...
					main_call_func = key_config_state_03_fade_init;
				}
				else
				if (psp_pad.pad_data & PSP_KEY_SHOT_OK) // ショットボタン入力
				{
...
static void key_config_state_01_fade_in(void)
{
	...
	if ((/*250-6*/224) < bg_alpha_aaa)
	{
		bg_alpha_aaa = (255);
		psp_push_screen();
		main_call_func = key_config_state_02_select_left_menu;
	}
	SDL_SetAlpha(sdl_screen[SDL_01_BACK_SCREEN], SDL_SRCALPHA, bg_alpha_aaa);
	psp_pop_screen();
}
...
global void key_config_start(void)
{
	...
	bg_alpha_aaa				= (0);
	main_call_func = key_config_state_01_fade_in;
}
