			...
			else	/* 上下以外の通常処理(斜め入力を排除する為、上下の入力があった場合はそれ以外の処理をしない) */
			{
				...
				if (OPTION_MENU_09_QUIT == menu_cursor1)	// 項目[ QUIT ] を選んでいる場合
				{
					if (psp_pad.pad_data & PSP_KEY_BOMB_CANCEL) 	// キャンセルボタン入力
					{
						...
						main_call_func = option_menu_terminate;
					}
					else
					if (psp_pad.pad_data & PSP_KEY_SHOT_OK) 	// ショットボタン入力
					{
				...
				if (OPTION_MENU_08_KEYCONFIG == menu_cursor1)			// 項目[ KEY CONFIG ] を選んでいる場合
				{
					...
					{
						voice_play(VOICE07_BOMB, TRACK01_EXPLODE);/*テキトー*/
						menu_out_call_func = key_config_start;	/* キーコンフィグメニューへ移動指示 */
						main_call_func = option_menu_terminate;
					}
				}
				else
				if (OPTION_MENU_08_MUSIC_ROOM == menu_cursor1)			// 項目[ MUSIC ROOM ] を選んでいる場合
				{
					...
					{
						voice_play(VOICE07_BOMB, TRACK01_EXPLODE);/*テキトー*/
						menu_out_call_func = music_room_start;	/* ミュージックルームへ移動指示 */
						main_call_func = option_menu_terminate;
					}
				}
...
static void option_menu_state_00_slide_in(void)
{
	...
	//		bg_alpha_aaa = 255;
			slide_offset_x = 0;
//			psp_push_screen();
			main_call_func = option_menu_state_01_select_menu;
		}
//		SDL_SetAlpha(sdl_screen[SDL_01_BACK_SCREEN], SDL_SRCALPHA, bg_alpha_aaa);
		psp_pop_screen();
	}
...
//			"ANALOG",
	...
	//
//	menu_out_call_func = title_menu_start;		/* タイトルメニューへ移動指示 */
	main_call_func = option_menu_state_00_slide_in;
}
