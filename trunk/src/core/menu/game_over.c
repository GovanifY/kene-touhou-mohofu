...
//
	...
	*/
	sprite_remove_all_SDL_void();/*gu汎用*/ 	/*SP_GROUP_ALL_GAME_OBJS*/
	sprite_remove_all_444_void();/*弾幕専用*/	/*SP_GROUP_ALL_GAME_OBJS*/
...
//削除	名前入力で使う。	player_now_stage = 0;		// [***090702		追加
	...
	pd_state_flag &= (~(STATE_FLAG_13_DRAW_BOSS_GAUGE));	/* Guは書かないとマズイ */
	#if 1
...
//	strcpy(buffer,	"STAGE   0 X 1000 PTS.");	dec_print_format( player_now_stage, 1, (char *)&buffer[ 8]);	font_print_screen_xy(buffer, FONT16W, 8,  60);	/*原作風*/
//	strcpy(buffer,	"POWER 000 X  100 PTS.");	dec_print_format( pd_weapon_power,	3, (char *)&buffer[ 6]);	font_print_screen_xy(buffer, FONT16W, 8,  80);	/*原作風*/
//	strcpy(buffer,	"GRAZE 000 X   10 PTS.");	dec_print_format( pd_graze_point,	3, (char *)&buffer[ 6]);	font_print_screen_xy(buffer, FONT16W, 8, 100);	/*原作風*/
	//				"012345678901234567890
	strcpy(buffer,	"STAGE   0 X 10000PTS.");	dec_print_format( player_now_stage, 1, (char *)&buffer[ 8]);	font_print_screen_xy(buffer, FONT16W, 8,  60);	/*模倣風*/
	strcpy(buffer,	"GRAZE 000 X  1000PTS.");	dec_print_format( pd_graze_point,	3, (char *)&buffer[ 6]);	font_print_screen_xy(buffer, FONT16W, 8,  80);	/*模倣風*/
	strcpy(buffer,	"POWER 000 X   100PTS.");	dec_print_format( pd_weapon_power,	3, (char *)&buffer[ 6]);	font_print_screen_xy(buffer, FONT16W, 8, 100);	/*模倣風*/
...
//	strcpy(buffer,	"GAME OVER" );																			font_print_screen_xy(buffer, FONT16R, 0,  32);
	...
	strcpy(buffer,	"SCORE     0000000000.");	dec_print_format( pd_my_score,		9, (char *)&buffer[10]);	font_print_screen_xy(buffer, FONT16W, 8,  60);
	strcpy(buffer,	"TOTAL MISTAKE      0.");	dec_print_format( pd_count_miss,	3, (char *)&buffer[17]);	font_print_screen_xy(buffer, FONT16W, 8,  80);	/* ミス回数 */
	strcpy(buffer,	"USE BOMBS          0.");	dec_print_format( pd_used_bomber,	3, (char *)&buffer[17]);	font_print_screen_xy(buffer, FONT16W, 8, 100);	/* ボム使用回数 */
	strcpy(buffer,	"BOMB WITH REVIVAL  0.");	dec_print_format( pd_use_kurai_bomb, 3, (char *)&buffer[17]);	font_print_screen_xy(buffer, FONT16W, 8, 120);	/* 喰らいボム成功回数 */
	strcpy(buffer,	"USE CONTINUE       0.");	dec_print_format( pd_use_continue,	3, (char *)&buffer[17]);	font_print_screen_xy(buffer, FONT16W, 8, 140);	/* コンティニュー回数 */
