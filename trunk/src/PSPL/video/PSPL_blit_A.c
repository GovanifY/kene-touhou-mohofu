/*
http://psp.jim.sh/svn/filedetails.php?repname=psp&path=%2Ftrunk%2FSDL%2Fsrc%2Fvideo%2FSDL_blit_A.c

psp - リビジョン 441Subversion リポジトリ一覧: pspps2ps2wareps3ps3warepspwareリビジョン:(root)/trunk/SDL/src/video/SDL_blit_A.c
...
#ifdef SAVE_RCSID
static char rcsid =
 "@(#) $Id: PSPL_blit_A.c,v 1.10 2005/04/17 10:19:22 icculus Exp $";
#endif
...
static void Blit16to16SurfaceAlpha128(SDL_BlitInfo *info, u16 mask)
{
	int width		= info->d_width;
	int height		= info->d_height;
	...
	while (height--)
	{
		if (((u32)srcp ^ (u32)dstp) & 2)
		{
			...
			while (w > 1)
			{
				...
				{	s = (prev_sw >> 16) + (sw << 16);	}
				prev_sw = sw;
				*(u32 *)dstp = BLEND2x16_50(dw, s, mask);
				dstp += 2;
				srcp += 2;
				w -= 2;
			}
		...
		else
		{
			...
			while (w > 1)
			{
				u32 sw = *(u32 *)srcp;
				u32 dw = *(u32 *)dstp;
				*(u32 *)dstp = BLEND2x16_50(dw, sw, mask);
				srcp += 2;
				dstp += 2;
				w -= 2;
			}
