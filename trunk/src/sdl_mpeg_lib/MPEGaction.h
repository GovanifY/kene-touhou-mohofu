...
public:
	...
	virtual /*dou ble*/float Time(void) /* Returns the time in seconds since start */
	{
		return (play_time);
	}
	...
	virtual void ResetSynchro(/*dou ble*/float time) = 0;
	virtual void Skip(float seconds) = 0;
	virtual void Pause(void)	/* A toggle action */
	{
		if ( paused )	{	paused = false; 	Play(); 	}
		else			{	Stop(); 		paused = true;	}
	}
...
public:
	...
	//volume = 0;//virtual void Volume(int vol) = 0;
};
