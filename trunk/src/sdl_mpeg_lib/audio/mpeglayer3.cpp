...
#include <math.h>
#include "psp_vfpu.h"/* ng(起動しない???) */

#include <stdlib.h>

#include "MPEGaudio.h"
#if defined(_WIN32) && defined(_MSC_VER)
	// disable warnings about dou ble to float conversions
...
//#define PI	(REAL)(M_PI)
#define PI_12	(REAL)(M_PI/12.0f)
#define PI_18	(REAL)(M_PI/18.0f)
#define PI_24	(REAL)(M_PI/24.0f)
#define PI_36	(REAL)(M_PI/36.0f)
#define PI_72	(REAL)(M_PI/72.0f)
...
#if 0
//static REAL cos1_6 = /*vf pu_cosf*/cos/*cos*/((REAL)(M_PI/6.0*1.0));/* gccは最適化してくれない(VisualC++は(定数)最適化する) */	/* ng? vfpu_cosf */
//static REAL cos2_6 = /*vf pu_cosf*/cos/*cos*/((REAL)(M_PI/6.0*2.0));/* gccは最適化してくれない(VisualC++は(定数)最適化する) */	/* ng? vfpu_cosf */
/*
	1. VisualC++ の場合。
		基本的にこの手の初期化は最適化にも拠るがdouble定数になる。
		初期化しか使わないなら mathlib の該当関数部分はリンクされない。

	2. psp の gcc の場合。
		どんな最適化をかけても 定数にならない。
		実行前の初期化フェイズ(ランタイム初期化)がある。
		mathlib の該当関数部分が強制的にリンクされ、実行前の初期化フェイズ時に
		mathlib の該当関数を実際に実行して、出た値を嵌め込む初期化を行う。

	2. psp で vfpu 使う場合。
		ここにvfpu 使う関数を記述しても動作しない。
		実行前の初期化フェイズ時に実行して、くれるのなら上手くいきそうだが
		実際は上手くいかない。(出た値を嵌め込む部分はやってると思うが)
		ＯＳとの兼ね合いかもしれない。(初期化フェイズ時にvfpuが初期化されてない可能性)
		初期化の順序が悪いのかもしれない。
*/
#endif
...
static REAL hsec_36[9];
static REAL hsec_12[3];
...
static RATIOS rat_1[16];
static RATIOS rat_2[2][64];
...
//	pow( 0.0, 1.333333f );
	...
	{
		for (i=0;i<18;i++)
		{	win[0][i] = win[1][i] = 0.5f * (vfpu_sinf/*sin*/(PI_72*(REAL)(2*i+1))) / (vfpu_cosf/*cos*/(PI_72*(REAL)(2*i+19)));}
		for (;i<36;i++)
		{	win[0][i] = win[3][i] = 0.5f * (vfpu_sinf/*sin*/(PI_72*(REAL)(2*i+1))) / (vfpu_cosf/*cos*/(PI_72*(REAL)(2*i+19)));}

		for (i=0;i<6;i++)
		{
			win[1][i+18] = 0.5f 										   / (vfpu_cosf/*cos*/(PI_72*(REAL)(2*(i+18)+19)));
			win[3][i+12] = 0.5f 										   / (vfpu_cosf/*cos*/(PI_72*(REAL)(2*(i+12)+19)));
			win[1][i+24] = 0.5f * (vfpu_sinf/*sin*/(PI_24*(REAL)(2*i+13))) / (vfpu_cosf/*cos*/(PI_72*(REAL)(2*(i+24)+19)));
			win[3][i+6 ] = 0.5f * (vfpu_sinf/*sin*/(PI_24*(REAL)(2*i+ 1))) / (vfpu_cosf/*cos*/(PI_72*(REAL)(2*(i+ 6)+19)));
			//
			win[1][i+30] =
			win[3][i   ] = 0.0f;
		}
		for (i=0;i<12;i++)
		{	win[2][i   ] = 0.5f * (vfpu_sinf/*sin*/(PI_24*(REAL)(2*i+ 1))) / (vfpu_cosf/*cos*/(PI_24*(REAL)(2*(i   )+ 7)));}
	}

	for (i=0;i<9;i++)
	{	cos_18[i]  = /*vf pu_cosf*/vfpu_cosf/*cos*/((REAL)PI_18*(/*dou ble*/float)(i));}/* ok! vfpu_cosf */
	for (i=0;i<9;i++)
	{	hsec_36[i] = 0.5f / /*vf pu_cosf*/vfpu_cosf/*cos*/((REAL)PI_36*(/*dou ble*/float)(i*2+1));}/* ok!(division zero?) vfpu_cosf */

	for (i=0;i<3;i++)
	{	hsec_12[i] = 0.5f / /*vf pu_cosf*/vfpu_cosf/*cos*/((REAL)PI_12*(/*dou ble*/float)(i*2+1));}/* ok!(division zero?) vfpu_cosf */

	for (i=0;i<40;i++)
//	{	two_to_negative_half_pow[i] = (REAL)/*vf pu_powf*/pow/*pow*/( (REAL)2.0f, (REAL)(-0.5f * (/*dou ble*/float)i) );}
	{	two_to_negative_half_pow[i] = (REAL)/*vf pu_powf*/vfpu_powf/*pow*/( (REAL)2.0f, (REAL)(-0.5f * (/*dou ble*/float)i) );}/* for my fixed custum lib only.(compatible mathlib) */
//	{	two_to_negative_half_pow[i] = (REAL)/*vf pu_powf*/vfpu_powf/*pow*/((REAL)(-0.5f * (/*dou ble*/float)i), (REAL)2.0f );}/* for PSPSDK(not compatible mathlib) */
...
//KR
		...
		{
			TO_FOUR_THIRDS[-i]= -(TO_FOUR_THIRDS[i] = (REAL)/*vf pu_powf*/vfpu_powf/*pow*/( (REAL)i, (REAL)(4.0f/3.0f)));/* for my fixed custum lib only.(compatible mathlib) */
		}
	}

	for (i=0; i<256; i++)
	{	POW2[i] = (REAL)/*vf pu_powf*/vfpu_powf/*pow*/(2.0f, (REAL)(0.25f*(i-210.0f)) );}/* for my fixed custum lib only.(compatible mathlib) */
	for (i=0; i<8; i++)
	{	for (j=0; j<2; j++)
		{	for (k=0; k<16; k++)
			{	POW2_1[i][j][k] = /*vf pu_powf*/vfpu_powf/*pow*/(2.0f, (REAL)((-2.0f*i)-(0.5f * (1.0f+j)*k)) );/* for my fixed custum lib only.(compatible mathlib) */
			}
		}
	}

	{
		static const REAL TAN12[16]=
		{
			0.0f,
			0.26794919f,
			0.57735027f,
...
#define IO0 ((REAL)0.840896415256f)
#define IO1 ((REAL)0.707106781188f)
	...
	for (i=1; i<64; i++)
	{
		...
		if ((i&(2-1))==1)
		{
			rat_2[0][i].l = /*vf pu_powf*/vfpu_powf/*pow*/(IO0, (REAL)((i+1)>>1) );/* for my fixed custum lib only.(compatible mathlib) */
			rat_2[1][i].l = /*vf pu_powf*/vfpu_powf/*pow*/(IO1, (REAL)((i+1)>>1) );/* for my fixed custum lib only.(compatible mathlib) */
			rat_2[0][i].r =
			rat_2[1][i].r = 1.0f;
		}
		else
		{
			rat_2[0][i].l =
			rat_2[1][i].l = 1.0f;
			rat_2[0][i].r = /*vf pu_powf*/vfpu_powf/*pow*/(IO0, (REAL)(i>>1) );/* for my fixed custum lib only.(compatible mathlib) */
			rat_2[1][i].r = /*vf pu_powf*/vfpu_powf/*pow*/(IO1, (REAL)(i>>1) );/* for my fixed custum lib only.(compatible mathlib) */
		}
	}
	{
		static REAL Ci[8] =
	...
	//	int i;
		for (i=0;i<8;i++)
		{
			sq = /*vfpu_sqrtf*/vfpu_sqrtf/*sqrt*/( (REAL)(1.0f+(Ci[i]*Ci[i]) ) );
			cs[i] = 1.0f / sq;
			ca[i] = Ci[i]*cs[i];
		}
	}
...
void MPEGaudio::huffmandecoder_2(
...
REAL MPEGaudio::layer3twopow2(
	int scale,int preflag,
...
void MPEGaudio::layer3dequantizesample(
	int ch,int gr,
...
void MPEGaudio::layer3fixtostereo(int gr,REAL in[2][SBLIMIT][SSLIMIT])
{
	...
	if (i_stereo)
	{
		...
		if (version)ratios=rat_2[((gi->scalefac_compress)&(2-1))/*%2*/];
		else ratios=rat_1;
...
{
//	/*static*/ const REAL cos1_6 = /*vf pu_cosf*/vfpu_cosf/*cos*/((REAL)(M_PI/6.0*1.0));/* gccは最適化してくれない(VisualC++は(定数)最適化する) */	/* ng? vfpu_cosf */
	/*static*/ const REAL cos1_6 = /*vf pu_cosf*/vfpu_cosf/*cos*/((REAL)(M_PI/6.0*1.0));/* gccは最適化してくれない(VisualC++は(定数)最適化する) */	/* ng? vfpu_cosf */
	/*static*/ const REAL cos2_6 = /*vf pu_cosf*/vfpu_cosf/*cos*/((REAL)(M_PI/6.0*2.0));/* gccは最適化してくれない(VisualC++は(定数)最適化する) */	/* ng? vfpu_cosf */
...
void MPEGaudio::extractlayer3(void)
{
...

	{
		...
		{	register int i;
			if (issync())
			{
				for (i=layer3slots; i>0; i--) // read main data.
