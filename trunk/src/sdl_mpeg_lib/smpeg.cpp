...
int SMPEG_playAudio( SMPEG* mpeg, u8 *stream, int len)
{
	MPEGaudio *audio = mpeg->obj->GetAudio();
	return (Play_MPEGaudio(audio, stream, len));
}
void SMPEG_playAudioSDL( void* mpeg, u8 *stream, int len)
{
...
int SMPEG_wantedSpec( SMPEG *mpeg, PSPL_AUDIO_SPEC *wanted )
{
	return ((int)mpeg->obj->WantedSpec(wanted));
}

/* Inform SMPEG of the actual SDL audio spec used for sound playback */
void SMPEG_actualSpec( SMPEG *mpeg, PSPL_AUDIO_SPEC *spec )
{
