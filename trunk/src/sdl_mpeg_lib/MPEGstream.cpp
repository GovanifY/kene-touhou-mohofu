...
MPEGstream::MPEGstream(MPEGsystem * System, u8 Streamid)
{
	system = System;
	streamid = Streamid;
	br = new MPEGlist();
...
bool MPEGstream:: next_system_buffer(void)
{
	bool has_data = true;
...
bool MPEGstream:: next_packet(bool recurse, bool update_timestamp)
{
	SDL_mutexP(mutex);
...
MPEGstream_marker *MPEGstream:: new_marker(int offset)
{
...
bool MPEGstream:: seek_marker(MPEGstream_marker const * marker)
{
	SDL_mutexP(mutex);
	if ( marker )
	{
...
void MPEGstream:: delete_marker(MPEGstream_marker *marker)
{
...
u32 MPEGstream:: copy_data(u8 *area, s32 size, bool short_read)
{
	u32 copied = 0;
...
void MPEGstream::insert_packet(u8 * Data, u32 Size, /*dou ble*/float timestamp)
{
	MPEGlist * newbr;
