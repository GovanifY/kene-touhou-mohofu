...
public:
	...
	/*dou ble*/float Time(void);
	void Play(void);
	void Stop(void);
	void Rewind(void);
	void ResetSynchro(/*dou ble*/float time);
	...
	bool WantedSpec(PSPL_AUDIO_SPEC *wanted);

	/* Inform SMPEG of the actual audio format if configuring SDL
	   outside of this class */
	void ActualSpec(const PSPL_AUDIO_SPEC *actual);
...
private:
	...
	static REAL hcos_64[16];
	static REAL hcos_32[8];
	static REAL hcos_16[4];
	static REAL hcos_8[2];
	static REAL hcos_4;
...
public:
	void initialize();
	bool run(int frames, /*dou ble*/float *timestamp = NULL);
...
private:
	...
	void layer3dequantizesample(int ch,int gr,
		int   in[SBLIMIT][SSLIMIT],
		REAL out[SBLIMIT][SSLIMIT]);
	void layer3fixtostereo(int gr,REAL	in[2][SBLIMIT][SSLIMIT]);
	void layer3reorderandantialias(int ch, int gr,
		REAL  in[SBLIMIT][SSLIMIT],
		REAL out[SBLIMIT][SSLIMIT]);

	void layer3hybrid(int ch,int gr,
		REAL  in[SBLIMIT][SSLIMIT],
...
public:
	...
	/*dou ble*/float/*TimeStamp*/ timestamp[N_TIMESTAMPS];

	/* Functions which access MPEGaudio internals */
	friend void Play_MPEGaudioSDL(void *udata, u8 *stream, int len);
	friend int Play_MPEGaudio(MPEGaudio *audio, u8 *stream, int len);
	#ifdef THREADED_AUDIO
...
void Play_MPEGaudioSDL(void *udata, u8 *stream, int len);
int Play_MPEGaudio(MPEGaudio *audio, u8 *stream, int len);
#ifdef THREADED_AUDIO
int Decode_MPEGaudio(void *udata);
#endif
