...
{
	#if 0//îpé~ ã»ÇÃÉRÉÅÉìÉgï∂éöóÒ:
	if (len)/* ÉRÉÅÉìÉgÇ™Ç†ÇÈèÍçá */
	{
		...
		/* translate IT linefeeds.(ITå`éÆÇÃèÍçáâ¸çsÇèCê≥Ç∑ÇÈÅB) */
		{
			int i;
			for (i=0; i<len; i++)
			{
		...
		mmoo.comment[len] = (0);	/* just in case */	/* [EOS] ï∂éöóÒèIí[ */
	}
	if (!mmoo.comment[0])/* ÉRÉÅÉìÉgÇ™ñ≥Ç¢èÍçá */
	{
		free(mmoo.comment);
		mmoo.comment = NULL;
	}
...
{
	MOD_VIRTUAL_MIXER_SampleUnload(handle);/*(ÇΩÇ‘ÇÒ)*/
}
#endif/*(1)*/
...
static void ML_XFreeSample(MM_SAMPLE *s)
{
	if (s->handle>=0)
	{
		MOD_VIRTUAL_MIXER_SampleUnload( (s16)(s->handle) );/*(ÇΩÇ‘ÇÒ)*/
	}
...
{
//îpé~		if (mf->songname)	{free(mf->songname);}
//îpé~		if (mf->comment)	{free(mf->comment);}
//îpé~		if (mf->modtype)	{free(mf->modtype);}
