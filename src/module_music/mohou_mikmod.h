...
MOD_MUSIC_API extern MM_s8* MusicMod_InfoDriver(void);

MOD_MUSIC_API extern int		MusicMod_DriverFromAlias(MM_s8*);

MOD_MUSIC_API extern MM_BOOL	MusicMod_Init(void/*MM_s8*cmdline*/);
MOD_MUSIC_API extern void		MusicMod_Exit(void);
MOD_MUSIC_API extern MM_BOOL	MusicMod_Reset(MM_s8*);
...
{
//	void/*MM_BOOL*/		(*Read_ void)(struct MF_READER_t*, void*, size_t);
...
typedef struct /*MM_SAMPLE _tag*/
{
	...
	u8		vibdepth;
	u8		vibrate;
	MM_s8*	samplename; /* name of the sample */
...
MOD_MUSIC_API extern MM_SAMPLE	*Sample_Load(MM_s8*);
...
typedef struct /*MM_INSTRUMENT _tag*/
{
	MM_s8*		insname;
	...
	u8			pitbeg;
	u8			pitend;
	MM_ENVPT	pitenv[ENVPOINTS];
} MM_INSTRUMENT;

struct MP_CONTROL_t;
struct MP_VOICE_t;
struct MOD_MUSIC_INTERNAL_MIDI_FILTER_tag;
...
/*
レゾナンスフィルターの設定。
IT形式では、midiで出来る事はなるべく出来るように作ってあるので、
midi形式でフィルターの設定ができるから、IT形式でもフィルターの設定ができます。
但しIT形式ではmidi形式のような豊富なフィルター形式は使用できません。
唯一利用できるのがレゾナンスフィルターです。
しかし、MikModではレゾナンスフィルターは実装してありません。
IT形式のレゾナンスフィルターの解釈部分までは実装してあります。
--------------------------------
レゾナンスフィルターは現在実装してないので、解釈部分を実装しても
レゾナンスフィルターを実装するまでは解釈部分は無意味なので、
これを飛ばすオプションを設けました。
0:USE_ITZ_ZXX_MIDI_RESONANT_FILTERS インプリメントされていないので、解釈部分を飛ばす。
1:USE_ITZ_ZXX_MIDI_RESONANT_FILTERS インプリメントされていないので、解釈部分を飛ばさない。
--------------------------------
pspではレゾナンスフィルターを実装しても速度的に地味に重いと思います。
弾幕シューとかじゃなければ、レゾナンスフィルターもそこそこ実用的なのですが。
*/
#define USE_ITZ_ZXX_MIDI_RESONANT_FILTERS (0)

typedef struct /*MOD_MUSIC_MODULE _tag*/
{

	/* general module information */
//廃止		MM_s8*			string_songname;	/* 曲名文字列: name of the song */
//廃止		MM_s8*			string_modtype; 	/* (MikMODが判別した)モジュール名文字列: string type of module loaded */
//廃止		MM_s8*			string_comment; 	/* 曲のコメント文字列: module comments */
	...
	MM_INSTRUMENT	*instruments;	/* all instruments */
	MM_SAMPLE		*samples;		/* all samples */
...
MOD_MUSIC_API extern MM_s8*  MusicMod_InfoLoader(void);
...
MOD_MUSIC_API extern MOD_MUSIC_MODULE* Player_Load(MM_s8*, int, MM_BOOL);
...
#ifdef USE_MIKMOD_ALONE
//MOD_MUSIC_API extern MM_s8*  Player_LoadTitle(MM_s8*);
#endif /* USE_MIKMOD_ALONE */
