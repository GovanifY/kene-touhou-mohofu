...
リビジョン 440
Subversion リポジトリ一覧: pspps2ps2wareps3ps3warepspwareリビジョン:(root)/trunk/SDL/src/thread/SDL_thread.c

http://psp.jim.sh/svn/filedetails.php?repname=psp&path=%2Ftrunk%2FSDL%2Fsrc%2Fthread%2Fpsp%2FSDL_systhread.c
リビジョン 2315
Subversion リポジトリ一覧: pspps2ps2wareps3ps3warepspwareリビジョン:(root)/trunk/SDL/src/thread/psp/SDL_systhread.c
リビジョン 1221
*/
...
static int s_SDL_SYS_CreateThread(SDL_Thread *thread, void *args)
{
	...
	thread->handle = sceKernelCreateThread(
		/*(このスレッドの名前)*/"SDL thread",
		/*(このスレッドの実位置[関数名])*/ThreadEntry,
		/*(このスレッドの優先度)*/priority, /*[値が小さい方が優先順位が高い])*/
		/*(このスレッドに割り当てるスタックの量)*/0x8000,
		/*(サウンドスレッド内でvfpuを使わない==0)*/PSP_THREAD_ATTR_VFPU,
		NULL);
	#else/*(模倣風)*/
	thread->handle = sceKernelCreateThread(
		/*(このスレッドの名前)*/"SDL thread",
		/*(このスレッドの実位置[関数名])*/ThreadEntry,
		/*(このスレッドの優先度)*/(8)/*priority*/, /*(模倣風ではサウンドスレッドを作るので実験的にプライオリティーを(32->8に)上げてみる[値が小さい方が優先順位が高い])*/
		/*(このスレッドに割り当てるスタックの量)*/(0x1000),/*(模倣風ではあまり大きなSDLスレッドは作らない)*/
		/*(サウンドスレッド内でvfpuを使わない==0)*/(0)/*PSP_THREAD_ATTR_VFPU*/,/*(模倣風ではSDLスレッド内でvfpuを使うプログラムは無い)*/
		NULL);
	#endif
	if (thread->handle < 0)
	{
