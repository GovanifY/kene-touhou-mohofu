 ...
 */
//[îpé~]static void in it_source(j_decompress_ptr cinfo)	//(jpegìWäJëOèàóùópÉRÅ[ÉãÉoÉbÉN)
//[îpé~]{
//[îpé~]	/* We don't actually need to do anything */
//[îpé~]	return;
//[îpé~]}
 ...
 */
//[îpé~]static void te rm_source(j_decompress_ptr cinfo)	//(jpegìWäJå„èàóùópÉRÅ[ÉãÉoÉbÉN)
//[îpé~]{
//[îpé~]	/* We don't actually need to do anything */
//[îpé~]	return;
//[îpé~]}
...
static void jpeg_SDL_RW_src(j_decompress_ptr cinfo, SDL_RWops *ctx)
{
	...
	src 						= (my_source_mgr *) cinfo->src;
//[îpé~]	src->pub.in it_source		= in it_source; 	//(jpegìWäJëOèàóùópÉRÅ[ÉãÉoÉbÉN)
	src->pub.fill_input_buffer	= fill_input_buffer;
	src->pub.skip_input_data	= skip_input_data;
	src->pub.resync_to_restart	= jpeg_resync_to_restart;	/* use default method */
//[îpé~]	src->pub.te rm_source		= te rm_source; 	//(jpegìWäJå„èàóùópÉRÅ[ÉãÉoÉbÉN)
	src->ctx					= ctx;
...
#else/*(ÉAÉìÉTÉ|Å[ÉgÇÃèÍçá)*/

/*(ÉAÉìÉTÉ|Å[ÉgÇÃèÍçáÇÕÉäÉìÉNÇµÇ»Ç¢ÇÃÇ≈é¿éøóvÇÁÇ»Ç¢)*/
//	/* See if an image is contained in a data source */
//	int IMG_isJPG(SDL_RWops *src)
//	{
//		return (0);/*(é∏îsÅAÇ±ÇÃÉtÉ@ÉCÉãÇÕjpegÇ≈ÇÕÇ»Ç¢)*/
//	}
//	/* Load a JPEG type image from an SDL datasource */
//	SDL_Surface *IMG_LoadJPG_RW(SDL_RWops *src)
//	{
//		return (NULL);/*(é∏îsÅAÇ±ÇÃÉtÉ@ÉCÉãÇÕì«Ç›çûÇﬂÇ»Ç©Ç¡ÇΩ)*/
//	}
