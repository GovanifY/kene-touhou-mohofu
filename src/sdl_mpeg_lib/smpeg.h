...
#include <SDL/SDL_mutex.h>//#include "SD L_mutex.h"
#include <SDL/SDL_audio.h>//#include "SD L_audio.h"
...
typedef struct _SMPEG_Info
{
	...
	int height;
	int current_frame;
/*info*/	/*dou ble*/float current_fps;
	char audio_string[80];
	int  audio_current_frame;
	u32 current_offset;
	u32 total_size;
/*info*/	/*dou ble*/float current_time;
/*info*/	/*dou ble*/float total_time;
} SMPEG_Info;
...
typedef void(*SMPEG_DisplayCallback)(
...
/*
   ...
   function.
 */
extern SMPEG* SMPEG_new_data(void *data, int size, SMPEG_Info* info, int sdl_audio);

/* The same for a generic SDL_RWops structure. */
extern SMPEG* SMPEG_new_rwops(SDL_RWops *src, SMPEG_Info* info, int sdl_audio);
...
*/
extern void SMPEG_setdisplay(
	SMPEG* mpeg, SDL_Surface* dst, SDL_mutex* surfLock,
...
extern void SMPEG_scaleXY( SMPEG* mpeg, int width, int height );
...
/* */
//#define SMPEG_dou_ble(mpeg, on) SMPEG_scale(mpeg, (on) ? 2 : 1)

/* Move the video display area within the destination surface */
extern void SMPEG_move( SMPEG* mpeg, int x, int y );

/* Set the region of the video to be shown */
extern void SMPEG_setdisplayregion(SMPEG* mpeg, int x, int y, int w, int h);
#endif
...
extern void SMPEG_seek( SMPEG* mpeg, int bytes);

/* Skip 'seconds' seconds in the MPEG stream */
extern void SMPEG_skip( SMPEG* mpeg, float seconds );
...
/* Set video filter */
extern SMPEG_Filter * SMPEG_filter( SMPEG* mpeg, SMPEG_Filter * filter );
#endif
...
*/
extern int SMPEG_playAudio( SMPEG *mpeg, u8 *stream, int len );

/* Wrapper for SMPEG_playAudio() that can be passed to SDL and SDL_mixer
*/
extern void SMPEG_playAudioSDL( void *mpeg, u8 *stream, int len );

/* Get the best SDL audio spec for the audio stream */
extern int SMPEG_wantedSpec( SMPEG *mpeg, PSPL_AUDIO_SPEC *wanted );

/* Inform SMPEG of the actual SDL audio spec used for sound playback */
extern void SMPEG_actualSpec( SMPEG *mpeg, PSPL_AUDIO_SPEC *spec );
