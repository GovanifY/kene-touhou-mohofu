...
static vorbis_info_floor *floor1_unpack (vorbis_info *vi,oggpack_buffer *opb)
{
	...
	for (j=0; j<maxclass+1; j++)
	{
		...
		if (info->class_book[j] < 0 ||
			info->class_book[j] >= ci->books)
		{
			goto err_out;
		}
...
static vorbis_look_floor *floor1_look(
	vorbis_dsp_state *vd,vorbis_info_mode *mi,
	vorbis_info_floor *in)
{
	...
	#else
	{	static const ogg_uint16_t aaa555[4] =
		{64,256,128,86};
		look->quant_q = aaa555[((info->mult)&0x03)];
	}
	#endif
...
static int floor1_inverse2(
	vorbis_block *vb,vorbis_look_floor *in,void *memo,
	ogg_int32_t *out)
{
	vorbis_look_floor1 *look=(vorbis_look_floor1 *)in;
	vorbis_info_floor1 *info=look->vi;
