...
STIN ogg_int32_t VFLOAT_MULT(
	ogg_int32_t 	a,	ogg_int32_t 	ap,
	...
	ogg_int32_t 	*p)
{
	...
	if (ap>bp)
	{
		...
		*p=ap+1;
		a>>=1;
		if (shift<32)	{	b=(b+(1<<(shift-1)))>>shift;	}
		else			{	b=0;	}
	}
	else
	{
		...
		*p=bp+1;
		b>>=1;
		if (shift<32)	{	a=(a+(1<<(shift-1)))>>shift;	}
		else			{	a=0;	}
	}
	a+=b;
