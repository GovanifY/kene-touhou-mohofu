...
static int vfpu_atani1024(float xxx)
{
	float result;
		...
		{
			{
				float y;	y=xxx;
				float x;	x=yyy;
				r1024 = vfpu_atani1024((float)(y/x));
			}
			if (yyy < 0 )
			{
				r1024 += (xxx < 0  ? -512 : 512 );
		...
		{
			{
				float y;	y=xxx;
				float x;	x=yyy;
				r1024 = vfpu_atani1024((float)(x/y));
			}
			r1024 = -r1024;
			r1024 += (xxx < 0  ? -(512 /2 ) : (512 /2 ));
		}
		#else
		#endif
	//	r512 = r512/**((512.0f)/(256.0f*2.0f))*/;
		r1024 += (1024*2);/* 必ず正値にする。 */
		r1024 &= (1024-1);
		return (r1024);
	}
#else
