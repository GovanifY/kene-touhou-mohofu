...
#ifndef _SPRITE_OBJECT_SYSTEM_H_
#define _SPRITE_OBJECT_SYSTEM_H_
...
#define SP_GROUP_BULLETS					(0x0400)/*(0x1000)0x0800*/		/* === Gu弾面 === */
...
//#define SP_GROUP_ALL_S DL_TYPE (0xff00)
//#define SP_GROUP_ALL_S DL_WORK_TYPE		(0x1f00)/* SDL動作 */
//#define SP_GROUP_ALL_S DL_CORE_TYPE		((0x1f00) & (~(SP_GROUP_PAUSE_S P_ME NU_TEXT|0)))/* game_core動作 */
#define SP_GROUP_ALL_SDL_CORE_TYPE			(0x1f00)/* game_core動作 */

//#define SP_GROUP_ALL_S DL_DRAW_TYPE		(SP_GROUP_ALL_S DL_WORK_TYPE & (~(SP_GROUP_TEKI|SP_GROUP_ITEMS|SP_GROUP_BULLETS)))/*SDL表示*/
/* 敵＆アイテム＆敵弾は少なくともS DLで描かない */
//#define SP_GROUP_ALL_S DL_DRAW_TYPE		(0x4000)/* SDL表示(現状SP_GROUP_PAUSE_S P_ME NU_TEXTのみS DL描画) */
...
//[4==1*4]
	...
	/*u8*/u32 flags;				/* フラグセット / (SP_FLAG...) */
	int type;						/* 種類及び使用可否 / Sprite-Type, ( enum SPRITE_TYPE), 0 = remove. */
//[8==2*4]
...
//[24==6*4]
	...
	void (*callback_move02_r35_dummy)(struct _sprite *src); 	/* Custom-Tamakesi-Routine / Zako Anime */
	void (*callback_move03)(struct _sprite *c, struct _sprite *src);
	void (*callback_move04)(struct _sprite *src);		/* Custom-loser-Routine (やられた後にボーナスを出すとか、違う爆発するとか) / 自機オプションのアニメーションコールバック */
//[28==7*4]
// 描画用
...
extern SPRITE *obj_collision_check_00_tama(SPRITE *tocheck, int type);/*gu弾専用*/

extern void gamen_gai_nara_zako_osimai(SPRITE *src);/* 画面外ならおしまい */
extern void check_tukaima_time_out(SPRITE *src);/* */

#if 1
/*---------------------------------------------------------
	弾幕システム
	-------------------------------------------------------
	弾幕システムはカードシステムやスペルシステムと全く関係無い概念。
	「弾幕」の文字通り、弾を単体としては認識せず、幕として認識するシステム。
	-------------------------------------------------------
---------------------------------------------------------*/
/*角度弾の喰み出しチェックを行う(毎フレーム行う必要はない)*/
/*画面外なら弾を消す*/
extern void danmaku_action_00_gamen_gai_nara_tama_wo_kesu(void);

/*画面外なら弾を反射*/
extern void danmaku_action_01_gamen_gai_nara_hansya_nomi(void); 	/* 反射弾のてすと */

/*画面外なら弾を反射減速*/
extern void danmaku_action_02_gamen_gai_nara_hansya_gensoku(void);	/* 反射減速弾のてすと */
#endif

/* 使用中であるが退避したい場合。画面外にスプライトを移動させ、無効にする。 */
extern void sprite_initialize_position(SPRITE *src);

#endif/* _SPRITE_OBJECT_SYSTEM_H_ */
