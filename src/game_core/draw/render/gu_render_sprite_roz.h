...
//(r32) unsigned int/*short*/ x_pos = ((spr->center.x256>>8)-(spr->w >> 1));/* 中心座標から画像サイズの半分を引き、左上座標を計算 */
//(r32) unsigned int/*short*/ y_pos = ((spr->center.y256>>8)-(spr->h >> 1));/* 中心座標から画像サイズの半分を引き、左上座標を計算 */
...
//		center_y = ((spr->center.y256>>8) 	) + (spr->h >> 1);/*/2*/
		...
		int zoom_x256;
		int zoom_y256;
	//	zoom_x256 = (spr->m_zoom.x256) * ((65536/256)); /* 拡大率は0-256倍なので0-65536倍へ変換。 */
	//	zoom_y256 = (spr->m_zoom.y256) * ((65536/256)); /* 拡大率は0-256倍なので0-65536倍へ変換。 */
		zoom_x256 = (spr->m_zoom.x256) /*<< (8)*/;		/* 拡大率は0-256倍なので0-65536倍へ変換。 */
		zoom_y256 = (spr->m_zoom.y256) /*<< (8)*/;		/* 拡大率は0-256倍なので0-65536倍へ変換。 */
		unsigned int j;
		for (j=0; j<4; j++)
		{
			int ifx;
