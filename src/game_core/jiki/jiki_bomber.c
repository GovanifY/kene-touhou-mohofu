...
//
	...
	#if (0)//
	src->cx256 = zzz_player->cx256 + ((si n1024((src->PL_KEKKAI_DATA_angleCCW1024))*(src->PL_KEKKAI_DATA_radius)) );/*fps_factor*/		/* CCWの場合 */
	src->cx256 = zzz_player->cx256 + ((si n1024((src->PL_KEKKAI_DATA_angleCCW1024))*(src->PL_KEKKAI_DATA_radius)) );	/* CCWの場合 */
	src->cy256 = zzz_player->cy256 + ((co s1024((src->PL_KEKKAI_DATA_angleCCW1024))*(src->PL_KEKKAI_DATA_radius)) );
	src->cy256 = zzz_player->cy256 + ((co s1024((src->PL_KEKKAI_DATA_angleCCW1024))*(src->PL_KEKKAI_DATA_radius)) );/*fps_factor*/
	#else
	{
		int sin_value_t256; 	//	sin_value_t256 = 0;
		int cos_value_t256; 	//	cos_value_t256 = 0;
		int256_sincos1024( (src->PL_KEKKAI_DATA_angleCCW1024), &sin_value_t256, &cos_value_t256);
		src->cx256 = zzz_player->cx256 + ((sin_value_t256*(src->PL_KEKKAI_DATA_radius)) );/*fps_factor*/
		src->cy256 = zzz_player->cy256 + ((cos_value_t256*(src->PL_KEKKAI_DATA_radius)) );/*fps_factor*/
	}
	#endif
...
//	for (angCCW1024=0; angCCW1024<(512); angCCW1024+=(512/3)/*(20)*/)
...
//		//	h->flags			|= (SP_FLAG_VISIBLE|SP_FLAG_TIME_OVER);
		...
		u32 my_rand;
			my_rand = ra_nd();
			{
				...
				h->cy256			= (96);//(u16)((my_rand>>16)) + t256((GAME_HEIGHT-256)/2);
			}
		...
		//	ang_aaa_1024 += ((my_rand	) & (0x3f));
			ang_aaa_1024 += ((my_rand>>8) & (0xff));
			ang_aaa_1024 += ((my_rand	) & (0xff));
		//	ang_aaa_1024 -= (0x3f);
		//	ang_aaa_1024 -= (0x1f);
			ang_aaa_1024 -= (0x5f);
			#if 0
			...
			#else
			ang_aaa_1024 &= (0x1ff);	/* 180/360 度の範囲に限定 */
			ang_aaa_1024 += (0x2ff);	/* 270/360 度回転(90/360度逆回転) */
			ang_aaa_1024 &= (0x3ff);	/* 360/360 度の範囲に限定 */
			#endif
			#if 1
			/* 描画用角度(下が0度で左回り(反時計回り)) */
			...
			#endif
			#if (0)//
			h->vx256		= ((si n1024((ang_aaa_1024))*(rnd_spd))>>8);/*fps_factor*/		/* CCWの場合 */
			h->vy256		= ((co s1024((ang_aaa_1024))*(rnd_spd))>>8);/*fps_factor*/
			#else
			{
				int sin_value_t256; 			//	sin_value_t256 = 0;
				int cos_value_t256; 			//	cos_value_t256 = 0;
				int256_sincos1024( (ang_aaa_1024), &sin_value_t256, &cos_value_t256);
				h->vx256		= ((sin_value_t256*(rnd_spd))>>8);/*fps_factor*/
				h->vy256		= ((cos_value_t256*(rnd_spd))>>8);/*fps_factor*/
			}
			#endif
			}
		}
...
//			h->rotationCCW1024		= ang_aaa_1024;
		...
		//	h->vy256		= (( ((src->vy256))*(rnd_spd))>>8);/*fps_factor*/
			h->vx256		= (src->vx256)/*<<(j)*/ ;/*fps_factor*/ /* CCWの場合 */
...
//	if (0x07==((src->jyumyou)&0x07))/* 波動を撃つタイミング(8回毎に11個) */
//	if (0x00==((src->jyumyou)&0x07))/* (-r34)波動を撃つタイミング			   (8回毎に11個) */
									/* (r35-)波動を撃つタイミング(1回毎に2個)==(8回毎に16個) */
	//
//	if (0x3f==(((int)pd_bomber_time)&0x3f))
//	if (0x0f==(((int)pd_bomber_time)&0x0f))
//	if (0x0f==((src->jyumyou)&0x0f))/* 波動を撃つタイミング(-r34) (16回毎に10x4==40個) */
//	if (0x00==((src->jyumyou)&0x01))/* 波動を撃つタイミング(r35-) (2回毎に4個==16回毎に8x4(32)個) */
//	if (0x00==((src->jyumyou)&0x01))/* 波動を撃つタイミング(r35-) (2回毎に3個==16回毎に8x3(24)個) */
									/* 波動を撃つタイミング(r35-) (1回毎に3個==16回毎に8x6(48)個) */
									/* 波動を撃つタイミング(r35-) (1回毎に2個==16回毎に8x4(32)個) */
//	SPRITE *zzz_player;
//	zzz_player = &obj99[OBJ_HEAD_02_KOTEI+FIX_OBJ_00_PLAYER];
...
{
	cg.state_flag	|= JIKI_FLAG_0x02_BOMBER_SYUU_SYUU; /* ボムによる自動収集可能 */
	int ii;//	int i;	/* 半象限ずつ */
...
/*static*/global void yuyuko_create_bomber_gyastry_dream_parrent(SPRITE *src)
{
	...
	//	if (NULL!=h)/* 登録できた場合のみ */
		if (NULL==h)return;/* 登録できた場合のみ */ 	/*うーん？？？*/
		{
			static const u8 base_bomber_atari[8] =
			{
	...
	for (iii=0; iii<(8)/*16*/; iii++)/*８方向*/
	{
		...
		//	if (NULL!=h)/* 登録できた場合のみ */
			...
			//	h->m_Hit256R			= ((16)<<8);//((base_bomber_atari[(cg_game_select_player)])<<8);
				...
				static const u32 color_mask32[8] =
				{	//		   RED	 GREEN BLUE  ALPHA
