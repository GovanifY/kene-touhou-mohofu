...
vorbis_dsp_state *vorbis_dsp_create(vorbis_info *vi)
{
	...
	{
		v->work[i]=(ogg_int32_t *)_ogg_calloc(1,(ci->blocksizes[1]>>1)* sizeof(*v->work[i]));
		v->mdctright[i]=(ogg_int32_t *)_ogg_calloc(1,(ci->blocksizes[1]>>2)* sizeof(*v->mdctright[i]));
	}
...
)
{
	...
	int postLap;
	{
		...
		int n1_2 = (n1>>2);
		int n0_2 = (n0>>2);
		r	= right+( (lW) ? (n1_2) : (n0_2));
		l	= in+((W_lW) ? (n1>>1) :	(n0>>1));
		wR	= ( (W_lW) ? w1+(n1>>1) : w0+(n0>>1));
		wL	= ( (W_lW) ? w1 	 : w0		 );

		halfLap 	= ( (W_lW)	? (n1_2) : (n0_2) );
		preLap		= ( (!W && lW)	? (n1_2)-(n0_2) : 0 );
		postLap 	= ( (W && !lW) ? (n1_2)-(n0_2) : 0 );
	}
	int n,off;
...
int vorbis_dsp_pcmout(vorbis_dsp_state *v,ogg_int16_t *pcm,int samples)
{
	...
	if ((v->out_begin)>(-1) && (v->out_begin)<(v->out_end))
	{
		...
		if (pcm)
		{
			...
			{
				s_mdct_unroll_lap(
					ci->blocksizes[0],ci->blocksizes[1],
					v->lW,v->W,
					v->work[i],
					v->mdctright[i],
...
int vorbis_dsp_synthesis(vorbis_dsp_state *vd,ogg_packet *op,int decodep)
{
	...
	if (decodep)
	{
		...
		{
			vd->out_end = (ci->blocksizes[vd->lW]>>2)+(ci->blocksizes[vd->W]>>2);/*/4*/
		}
		vd->out_begin = 0;
	}
  /* track the frame number... This is for convenience, but also
	...
	{
		vd->sample_count += (ci->blocksizes[vd->lW]>>2)+(ci->blocksizes[vd->W]>>2);/*/4*/
	}

	if (vd->granulepos==-1)
	{
		if (op->granulepos!=-1) /* only set if we have a position to set to */
		{
	...
	{
		vd->granulepos += (ci->blocksizes[vd->lW]>>2)+(ci->blocksizes[vd->W]>>2);/*/4*/
