...
static int _ov_open1(
	void *f,OggVorbis_File *vf,char *initial,
...
int ov_clear(OggVorbis_File *vf)
{
	if (vf)
	{
		vorbis_dsp_destroy(vf->vd);		vf->vd = 0;
...
int ov_open_callbacks(
	void *f,OggVorbis_File *vf,char *initial,long ibytes,
...
int ov_pcm_seek(OggVorbis_File *vf,ogg_int64_t pos)
{
	...
	//	int vorbis_dsp_read(vorbis_dsp_state *v,int s)
		{
			vorbis_dsp_state *v = vf->vd;
			if ((samples) && (v->out_begin+samples > v->out_end))
			{
			//	return (OV_EINVAL);
			}
			else
