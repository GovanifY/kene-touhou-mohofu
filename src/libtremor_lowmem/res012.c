		...
		if (n>0)
		{
			...
			if (used)
			{
				...
				{	partword[j] = (char *)alloca(
						partwords*partitions_per_word * sizeof(*partword[j]));
				}
				for (s=0; s<info->stages; s++)
				{
					for (i=0; i<partvals;)
					{
						if (s==0)
						{
							...
							for (j=0; j<ch; j++)
							{
								int temp = vorbis_book2_decode(phrasebook, &vd->opb);
						...
						for (k=0; k<partitions_per_word && i<partvals; k++,i++)
						{
							for (j=0; j<ch; j++)
							{
								...
								if (info->stagemasks[(int)partword[j][i]]&(1<<s))
								{
									...
									if (info->type)
									{
										if (vorbis_book2_decodev_add(
												stagebook, &vd->opb, samples_per_partition, -8, in[j]+offset )==-1)
										{	goto eopbreak;}
									}
									else
									{
										if (vorbis_book2_decodevs_add(
												stagebook, &vd->opb, samples_per_partition, -8, in[j]+offset )==-1)
										{	goto eopbreak;}
									}
								}
			...
			for (s=0; s<info->stages; s++)
			{
				for (i=0; i<partvals;)
				{
					if (s==0)
					{
						...
						temp = vorbis_book2_decode(phrasebook, &vd->opb);
						if (temp==-1)goto eopbreak;
						/* this can be done quickly in assembly due to the quotient
							 always being at most six bits */
						for (k=0; k<partitions_per_word; k++)
					...
					{	if (info->stagemasks[(int)partword[i]]&(1<<s))
						{
							...
							if (vorbis_book2_decodevv_add(
								stagebook, &vd->opb, samples_per_partition, -8,
								in, i*samples_per_partition+beginoff, ch)==-1)
							{	goto eopbreak;}
						}
					}
				}
