...
int mapping_info_unpack(vorbis_info_mapping *info, vorbis_info *vi, oggpack_buffer *opb)
{
	...
	if (oggpack_read(opb,1))
	{
		...
		info->coupling = _ogg_malloc(info->coupling_steps*sizeof(*info->coupling));
	...
	for (i=0; i < info->submaps; i++)
	{
		...
		info->submaplist[i].floor	= oggpack_read(opb,8);	if (info->submaplist[i].floor	>= ci->floors  )	{goto err_out;}
		info->submaplist[i].residue = oggpack_read(opb,8);	if (info->submaplist[i].residue >= ci->residues)	{goto err_out;}
	}
	return (0);
//
err_out:
...
int mapping_inverse(vorbis_dsp_state *vd, vorbis_info_mapping *info)
{
	...
	int 				i;
	int 				j;
//	long				nnn_2 = ((ci->blocksizes[vd->W])/2);
	long				nnn_2 = ((ci->blocksizes[vd->W])>>1);
	...
	for (i=0;i<vi->channels;i++)
	{
		...
		{
			/* floor 1 */
			floormemo[i]=alloca(sizeof(*floormemo[i]) * floor1_memosize(ci->floor_param[floorno]));
			floormemo[i]=floor1_inverse1(vd,ci->floor_param[floorno],floormemo[i]);
		}
		else
		{
			/* floor 0 */
			floormemo[i]=alloca(sizeof(*floormemo[i]) * floor0_memosize(ci->floor_param[floorno]));
			floormemo[i]=floor0_inverse1(vd,ci->floor_param[floorno],floormemo[i]);
		}
		if (floormemo[i])
				{	nonzero[i]=1;	}
		else	{	nonzero[i]=0;	}
		memset(vd->work[i],0,sizeof(*vd->work[i])*nnn_2);
	}
	...
	for (i=info->coupling_steps-1;i>=0;i--)
	{
		...
		for (j=0;j<nnn_2;j++)
		{
			...
			if (mag>0)
			{
				if (ang>0)	{	pcmM[j]=mag;	pcmA[j]=mag-ang;	}
				else		{	pcmA[j]=mag;	pcmM[j]=mag+ang;	}
			}
			else
			{
				if (ang>0)	{	pcmM[j]=mag;	pcmA[j]=mag+ang;	}
	...
	for (i=0;i<vi->channels;i++)
	{
		...
		floorno=info->submaplist[submap].floor;
		if (ci->floor_type[floorno])
				{	/* floor 1 */	floor1_inverse2(vd,ci->floor_param[floorno],floormemo[i],pcm);	}
		else	{	/* floor 0 */	floor0_inverse2(vd,ci->floor_param[floorno],floormemo[i],pcm);	}
	}
	...
	/* only MDCT right now.... */
	for (i=0;i<vi->channels;i++)
	{	mdct_backward((nnn_2+nnn_2),vd->work[i]);}
